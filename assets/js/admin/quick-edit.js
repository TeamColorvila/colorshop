jQuery(document).ready(function(){  
    jQuery('#the-list').on('click', '.editinline', function(){  
		...
		var $cs_inline_data = jQuery('#colorshop_inline_' + post_id );
		
		var sku 				= $cs_inline_data.find('.sku').text();
		var regular_price 		= $cs_inline_data.find('.regular_price').text();
		var sale_price 			= $cs_inline_data.find('.sale_price').text();
		var weight 				= $cs_inline_data.find('.weight').text();
		var length 				= $cs_inline_data.find('.length').text();
		var width 				= $cs_inline_data.find('.width').text();
		var height	 			= $cs_inline_data.find('.height').text();
		var visibility	 		= $cs_inline_data.find('.visibility').text();
		var stock_status	 	= $cs_inline_data.find('.stock_status').text();
		var stock	 			= $cs_inline_data.find('.stock').text();
		var featured	 		= $cs_inline_data.find('.featured').text();
		var manage_stock		= $cs_inline_data.find('.manage_stock').text();
		var menu_order			= $cs_inline_data.find('.menu_order').text();
		...
		// Conditional display
		var product_type		= $cs_inline_data.find('.product_type').text();
		var product_is_virtual	= $cs_inline_data.find('.product_is_virtual').text();
    ...
    jQuery('#wpbody').on('click', '#doaction, #doaction2', function(){  
		...
		jQuery('#colorshop-fields-bulk .inline-edit-group .alignright').hide();
		
	});
	
	 jQuery('#wpbody').on('change', '#colorshop-fields-bulk .inline-edit-group .change_to', function(){  
