jQuery( function($){
	...
	// Type box
	$('.type_box').appendTo( '#colorshop-product-data h3.hndle span' );

	$(function(){
		// Prevent inputs in meta box headings opening/closing contents
		$('#colorshop-product-data h3.hndle').unbind('click.postboxes');

		jQuery('#colorshop-product-data').on('click', 'h3.hndle', function(event){

			// If the user clicks on some form input inside the h3 the box should not be toggled
			if ( $(event.target).filter('input, option, label, select').length )
				return;

			$('#colorshop-product-data').toggleClass('closed');
		});
	});
	...
	$('#catalog-visibility .save-post-visibility').click(function () {
		...
		if ( $('input[name=_featured]').is(':checked') ) {
			label = label + ', ' + colorshop_writepanel_params.featured_label
	...
	$('ul.cs-tabs a').click(function(){
		var panel_wrap =  $(this).closest('div.panel-wrap');
		$('ul.cs-tabs li', panel_wrap).removeClass('active');
	...
	jQuery("select.ajax_chosen_select_products").ajaxChosen({
	    method: 	'GET',
	    url: 		colorshop_writepanel_params.ajax_url,
	    dataType: 	'json',
	    afterTypeDelay: 100,
	    data:		{
	    	action: 		'colorshop_json_search_products',
			security: 		colorshop_writepanel_params.search_products_nonce
	    }
	}, function (data) {
	...
	jQuery("select.ajax_chosen_select_products_and_variations").ajaxChosen({
	    ...
	    afterTypeDelay: 100,
	    data:		{
	    	action: 		'colorshop_json_search_products_and_variations',
			security: 		colorshop_writepanel_params.search_products_nonce
	    }
	}, function (data) {
	...
	// ORDERS
	jQuery('#colorshop-order-actions input, #colorshop-order-actions a').click(function(){
	...
	$('#colorshop-order-totals').on( 'change', '#_order_tax, #_order_shipping_tax, #_cart_discount, #_order_discount', function() {
		...
		var formatted_total = accounting.formatMoney( total, {
			symbol 		: colorshop_writepanel_params.currency_format_symbol,
			decimal 	: colorshop_writepanel_params.currency_format_decimal_sep,
			thousand	: colorshop_writepanel_params.currency_format_thousand_sep,
			precision 	: colorshop_writepanel_params.currency_format_num_decimals,
			format		: colorshop_writepanel_params.currency_format
		} );
	...
	$('button.calc_line_taxes').click(function(){
		// Block write panel
		$('.colorshop_order_items_wrapper').block({ message: null, overlayCSS: { background: '#fff url(' + colorshop_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

		var answer = confirm(colorshop_writepanel_params.calc_line_taxes);
		...
		if (answer) {
			...
			var data = {
				action: 		'colorshop_calc_line_taxes',
				order_id: 		colorshop_writepanel_params.post_id,
				...
				postcode:		postcode,
				city:			city,
				security: 		colorshop_writepanel_params.calc_totals_nonce
			};

			$.post( colorshop_writepanel_params.ajax_url, data, function( response ) {
				...
				$('.colorshop_order_items_wrapper').unblock();
			});
	...
	$('button.calc_totals').click( function(){
		// Block write panel
		$('#colorshop-order-totals').block({ message: null, overlayCSS: { background: '#fff url(' + colorshop_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

		var answer = confirm(colorshop_writepanel_params.calc_totals);
		...
		if (answer) {
			...
			$('#order_items_list tr.item').each(function(){
				...
				if ( colorshop_writepanel_params.round_at_subtotal=='no' ) {
			...
			// Tax
			if (colorshop_writepanel_params.round_at_subtotal=='yes') {
			...
			$('#colorshop-order-totals').unblock();
		...
		$('#colorshop-order-totals .calculated').css('background-color', '#e3d2dd');
	}, function() {
		$('#colorshop-order-totals .calculated').css('background-color', '');
	});

	// Add a line item
	$('#colorshop-order-items button.add_order_item').click(function(){
		...
		if ( add_item_ids ) {
			...
			$('table.colorshop_order_items').block({ message: null, overlayCSS: { background: '#fff url(' + colorshop_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

			$.each( add_item_ids, function( index, value ) {

				var data = {
					action: 		'colorshop_add_order_item',
					item_to_add: 	value,
					order_id:		colorshop_writepanel_params.post_id,
					security: 		colorshop_writepanel_params.order_item_nonce
				};

				$.post( colorshop_writepanel_params.ajax_url, data, function( response ) {

					$('table.colorshop_order_items tbody#order_items_list').append( response );
					...
					if (!--count) {
					    ...
					    $('table.colorshop_order_items').unblock();
					}
	...
	// Add a fee
	$('#colorshop-order-items button.add_order_fee').click(function(){

		$('table.colorshop_order_items').block({ message: null, overlayCSS: { background: '#fff url(' + colorshop_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

		var data = {
			action: 		'colorshop_add_order_fee',
			order_id:		colorshop_writepanel_params.post_id,
			security: 		colorshop_writepanel_params.order_item_nonce
		};
	...
	$('#order_items_list').on('click', 'button.add_order_item_meta', function(){
		...
		var data = {
			...
			action: 	'colorshop_add_order_item_meta',
			security: 	colorshop_writepanel_params.order_item_nonce
		};
	...
	$('#order_items_list').on('click', 'button.remove_order_item_meta', function(){
		var answer = confirm( colorshop_writepanel_params.remove_item_meta )
		if ( answer ) {
			var $row = $(this).closest('tr');

			var data = {
				meta_id: 			$row.attr( 'data-meta_id' ),
				action: 			'colorshop_remove_order_item_meta',
				security: 			colorshop_writepanel_params.order_item_nonce
			};
	...
	$('#colorshop-order-items').on( 'click', 'input.check-column', function() {
		if ( $(this).is(':checked') )
			$('#colorshop-order-items').find('.check-column input').attr('checked', 'checked');
		else
			$('#colorshop-order-items').find('.check-column input').removeAttr('checked');
	} );

	$('#colorshop-order-items').on( 'click', '.do_bulk_action', function() {

		var action = $(this).closest('.bulk_actions').find('select').val();
		var selected_rows = $('#colorshop-order-items').find('.check-column input:checked');
		var item_ids = [];
		...
		if ( item_ids.length == 0 ) {
			alert( colorshop_writepanel_params.i18n_select_items );
			return;
		}

		if ( action == 'delete' ) {

			var answer = confirm( colorshop_writepanel_params.remove_item_notice );
			...
			if ( answer ) {
				...
				var data = {
					...
					action: 			'colorshop_remove_order_item',
					security: 			colorshop_writepanel_params.order_item_nonce
				};
			...
			var data = {
				...
				action: 			'colorshop_reduce_order_item_stock',
				security: 			colorshop_writepanel_params.order_item_nonce
			};
			...
			var data = {
				...
				order_item_ids: 	item_ids,
				order_item_qty: 	quantities,
				action: 			'colorshop_increase_order_item_stock',
				security: 			colorshop_writepanel_params.order_item_nonce
			};
	...
	$('button.load_customer_billing').click(function(){
		...
		var answer = confirm(colorshop_writepanel_params.load_billing);
		if (answer){
			...
			if (!user_id) {
				alert(colorshop_writepanel_params.no_customer_selected);
				return false;
			}
			...
			var data = {
				...
				action: 			'colorshop_get_customer_details',
				security: 			colorshop_writepanel_params.get_customer_details_nonce
			};

			$(this).closest('.edit_address').block({ message: null, overlayCSS: { background: '#fff url(' + colorshop_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });
	...
	$('button.load_customer_shipping').click(function(){
		...
		var answer = confirm(colorshop_writepanel_params.load_shipping);
		if (answer){
	...
	$('button.billing-same-as-shipping').click(function(){
		var answer = confirm(colorshop_writepanel_params.copy_billing);
		if (answer){
	...
	$('a.add_tax_row').click(function(){
		...
		var data = {
			order_id: 	colorshop_writepanel_params.post_id,
			action: 	'colorshop_add_line_tax',
			security: 	colorshop_writepanel_params.calc_totals_nonce
		};

		$('#tax_rows').closest('.totals_group').block({ message: null, overlayCSS: { background: '#fff url(' + colorshop_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });
	...
	$('#tax_rows').on('click','a.delete_tax_row',function(){
		...
		var data = {
			...
			action: 	'colorshop_remove_line_tax',
			security: 	colorshop_writepanel_params.calc_totals_nonce
		};
	...
	$('select#product-type').change(function(){
		...
		$('ul.cs-tabs li:visible').eq(0).find('a').click();

		$('body').trigger('colorshop-product-type-change', select_val, $(this) );
	}).change();
	...
	function show_and_hide_panels() {
		...
		$.each( colorshop_writepanel_params.product_types, function( index, value ) {
	...
	$('#colorshop-product-data').on( 'click', '.sale_schedule', function() {
	...
	$('#colorshop-product-data').on( 'click', '.cancel_sale_schedule', function() {
		var $wrap = $(this).closest( 'div, table' );
	...
	var dates = $( ".sale_price_dates_fields input" ).datepicker({
		...
		showOn: "button",
		buttonImage: colorshop_writepanel_params.calendar_image,
	...
	// META BOXES
		...
		// Open/close
		jQuery('.cs-metaboxes-wrapper').on('click', '.cs-metabox h3', function(event){
			// If the user clicks on some form input inside the h3, like a select list (for variations), the box should not be toggled
			if ($(event.target).filter(':input, option').length) return;

			jQuery(this).next('.cs-metabox-content').toggle();
		})
		.on('click', '.expand_all', function(event){
			jQuery(this).closest('.cs-metaboxes-wrapper').find('.cs-metabox > table').show();
			return false;
		})
		.on('click', '.close_all', function(event){
			jQuery(this).closest('.cs-metaboxes-wrapper').find('.cs-metabox > table').hide();
			return false;
		});

		jQuery('.cs-metabox.closed').each(function(){
			jQuery(this).find('.cs-metabox-content').hide();
		});

	// ATTRIBUTE TABLES

		// Multiselect attributes
		$(".colorshop_attributes select.multiselect").chosen();

		// Initial order
		var colorshop_attribute_items = $('.colorshop_attributes').find('.colorshop_attribute').get();

		colorshop_attribute_items.sort(function(a, b) {
		   var compA = parseInt($(a).attr('rel'));
		   var compB = parseInt($(b).attr('rel'));
		   return (compA < compB) ? -1 : (compA > compB) ? 1 : 0;
		})
		$(colorshop_attribute_items).each( function(idx, itm) { $('.colorshop_attributes').append(itm); } );

		function attribute_row_indexes() {
			$('.colorshop_attributes .colorshop_attribute').each(function(index, el){
				$('.attribute_position', el).val( parseInt( $(el).index('.colorshop_attributes .colorshop_attribute') ) );
			});
		};
		...
		$('button.add_attribute').on('click', function(){
			...
			var size = $('.colorshop_attributes .colorshop_attribute').size();
			...
			if (!attribute_type) {
				...
				$('.colorshop_attributes').append('<div class="colorshop_attribute cs-metabox">\
						<h3>\
							<button type="button" class="remove_row button">' + colorshop_writepanel_params.remove_label + '</button>\
							<div class="handlediv" title="' + colorshop_writepanel_params.click_to_toggle + '"></div>\
							<strong class="attribute_name"></strong>\
						</h3>\
						<table cellpadding="0" cellspacing="0" class="colorshop_attribute_data">\
							<tbody>\
								<tr>\
									<td class="attribute_name">\
										<label>' + colorshop_writepanel_params.name_label + ':</label>\
									...
									<td rowspan="3">\
										<label>' + colorshop_writepanel_params.values_label + ':</label>\
										<textarea name="attribute_values[' + size + ']" cols="5" rows="5" placeholder="' + colorshop_writepanel_params.text_attribute_tip + '"></textarea>\
									</td>\
								</tr>\
								<tr>\
									<td>\
										<label><input type="checkbox" class="checkbox" ' + ( colorshop_writepanel_params.default_attribute_visibility ? 'checked="checked"' : '' ) + ' name="attribute_visibility[' + size + ']" value="1" /> ' + colorshop_writepanel_params.visible_label + '</label>\
									</td>\
								</tr>\
								<tr>\
									<td>\
										<div class="enable_variation show_if_variable" ' + enable_variation + '>\
										<label><input type="checkbox" class="checkbox" ' + ( colorshop_writepanel_params.default_attribute_variation ? 'checked="checked"' : '' ) + ' name="attribute_variation[' + size + ']" value="1" /> ' + colorshop_writepanel_params.used_for_variations_label + '</label>\
										</div>\
									</td>\
								</tr>\
							</tbody>\
				...
				// Reveal taxonomy row
				var thisrow = $('.colorshop_attributes .colorshop_attribute.' + attribute_type);
				$('.colorshop_attributes').append( $(thisrow) );
				$(thisrow).show().find('.colorshop_attribute_data').show();
		...
		$('.colorshop_attributes').on('blur', 'input.attribute_name', function(){
			$(this).closest('.colorshop_attribute').find('strong.attribute_name').text( $(this).val() );
		});

		$('.colorshop_attributes').on('click', 'button.select_all_attributes', function(){
			$(this).closest('td').find('select option').attr("selected","selected");
			$(this).closest('td').find('select').trigger("liszt:updated");
			return false;
		});

		$('.colorshop_attributes').on('click', 'button.select_no_attributes', function(){
			$(this).closest('td').find('select option').removeAttr("selected");
			$(this).closest('td').find('select').trigger("liszt:updated");
			return false;
		});

		$('.colorshop_attributes').on('click', 'button.remove_row', function() {
			var answer = confirm(colorshop_writepanel_params.remove_attribute);
			if (answer){
		...
		// Attribute ordering
		$('.colorshop_attributes').sortable({
			items:'.colorshop_attribute',
			cursor:'move',
			axis:'y',
			handle: 'h3',
		...
		$('.colorshop_attributes').on('click', 'button.add_new_attribute', function() {

			$('.colorshop_attributes').block({ message: null, overlayCSS: { background: '#fff url(' + colorshop_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

			var attribute = $(this).attr('data-attribute');
			var $wrapper = $(this).closest('.colorshop_attribute_data');
			var new_attribute_name = prompt( colorshop_writepanel_params.new_attribute_prompt );

			if ( new_attribute_name ) {

				var data = {
					action: 		'colorshop_add_new_attribute',
					taxonomy:		attribute,
					term:			new_attribute_name,
					security: 		colorshop_writepanel_params.add_attribute_nonce
				};
		...
		$('.save_attributes').on('click', function(){
			...
			var data = {
				post_id: 		colorshop_writepanel_params.post_id,
				data:			$('.colorshop_attributes').find('input, select, textarea').serialize(),
				action: 		'colorshop_save_attributes',
				security: 		colorshop_writepanel_params.save_attributes_nonce
			};
			...
			$.post( colorshop_writepanel_params.ajax_url, data, function( response ) {
				...
				this_page = this_page.replace( 'post-new.php?', 'post.php?post=' + colorshop_writepanel_params.post_id + '&action=edit&' );

				// Load variations panel
				$('#variable_product_options').block({ message: null, overlayCSS: { background: '#fff url(' + colorshop_writepanel_params.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });
