jQuery(document).ready(function($) {
	...
	function update_checkout() {
		...
		$('#order_methods, #order_review').block({message: null, overlayCSS: {background: '#fff url(' + colorshop_params.ajax_loader_url + ') no-repeat center', backgroundSize: '16px 16px', opacity: 0.6}});

		var data = {
			action: 			'colorshop_update_order_review',
			security: 			colorshop_params.update_order_review_nonce,
	...
	if ( colorshop_params.option_guest_checkout == 'yes' ) {
	...
	.on( 'blur change', '.input-text, select', function() {
		...
		if ( $parent.is( '.validate-required' ) ) {
			if ( $this.val() == '' ) {
				$parent.removeClass( 'colorshop-validated' ).addClass( 'colorshop-invalid colorshop-invalid-required-field' );
				validated = false;
			}
		}
		...
		if ( $parent.is( '.validate-email' ) ) {
			if ( $this.val() ) {
				...
				if ( ! pattern.test( $this.val()  ) ) {
					$parent.removeClass( 'colorshop-validated' ).addClass( 'colorshop-invalid colorshop-invalid-email' );
					validated = false;
				}
			}
		}
		...
		if ( validated ) {
			$parent.removeClass( 'colorshop-invalid colorshop-invalid-required-field' ).addClass( 'colorshop-validated' );
		}
	} )
	...
	.submit( function() {
		...
		if ( $form.triggerHandler('checkout_place_order') !== false && $form.triggerHandler('checkout_place_order_' + $('#order_review input[name=payment_method]:checked').val() ) !== false ) {
			...
			if ( form_data["blockUI.isBlocked"] != 1 )
				$form.block({message: null, overlayCSS: {background: '#fff url(' + colorshop_params.ajax_loader_url + ') no-repeat center', backgroundSize: '16px 16px', opacity: 0.6}});

			$.ajax({
				type: 		'POST',
				url: 		colorshop_params.checkout_url,
				data: 		$form.serialize(),
				success: 	function( code ) {
						try {
							// Get the valid JSON only from the returned string
							if ( code.indexOf("<!--CS_START-->") >= 0 )
								code = code.split("<!--CS_START-->")[1]; // Strip off before after CS_START

							if ( code.indexOf("<!--CS_END-->") >= 0 )
								code = code.split("<!--CS_END-->")[0]; // Strip off anything after CS_END
							...
							if (result.result=='success') {
								...
								$('.colorshop-error, .colorshop-message').remove();
	...
	$('form.checkout_coupon').submit( function() {
		...
		$form.addClass('processing').block({message: null, overlayCSS: {background: '#fff url(' + colorshop_params.ajax_loader_url + ') no-repeat center', backgroundSize: '16px 16px', opacity: 0.6}});

		var data = {
			action: 			'colorshop_apply_coupon',
			security: 			colorshop_params.apply_coupon_nonce,
	...
	/* Localisation */
	var locale_json = colorshop_params.locale.replace(/&quot;/g, '"');
	var locale = $.parseJSON( locale_json );
	var required = ' <abbr class="required" title="' + colorshop_params.i18n_required_text + '">*</abbr>';
	...
	// Update on page load
	if ( colorshop_params.is_checkout == 1 ) {
