...
if ( $existing_file_paths ) {
	...
	foreach( $existing_file_paths as $existing_file_path ) {
		...
		if ( ! empty( $old_file_path ) ) {
			...
			$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->prefix}colorshop_downloadable_product_permissions SET download_id = %s WHERE product_id = %d", md5( $old_file_path ), $existing_file_path->post_id ) );
...
$wpdb->query( "ALTER TABLE {$wpdb->prefix}colorshop_downloadable_product_permissions DROP PRIMARY KEY" );

$wpdb->query( "ALTER TABLE {$wpdb->prefix}colorshop_downloadable_product_permissions ADD PRIMARY KEY (  `product_id` ,  `order_id` ,  `order_key` ,  `download_id` )" );
...
if ( empty( $permalinks ) && $shop_page_id > 0 ) {
	...
	$category_base 	= get_option('colorshop_prepend_shop_page_to_urls') == "yes" ? trailingslashit( $base_slug ) : '';
	$category_slug 	= get_option('colorshop_product_category_slug') ? get_option('colorshop_product_category_slug') : _x( 'product-category', 'slug', 'colorshop' );
	$tag_slug 		= get_option('colorshop_product_tag_slug') ? get_option('colorshop_product_tag_slug') : _x( 'product-tag', 'slug', 'colorshop' );

	if ( 'yes' == get_option('colorshop_prepend_shop_page_to_products') ) {
		$product_base = trailingslashit( $base_slug );
	} else {
		if ( ( $product_slug = get_option('colorshop_product_slug') ) !== false && ! empty( $product_slug ) ) {
			$product_base = trailingslashit( $product_slug );
		} else {
			$product_base = trailingslashit( _x('product', 'slug', 'colorshop') );
		}
	}

	if ( get_option('colorshop_prepend_category_to_products') == 'yes' )
...
if ( get_option( 'colorshop_shop_show_subcategories' ) == 'yes' ) {
	if ( get_option( 'colorshop_hide_products_when_showing_subcategories' ) == 'yes' ) {
		update_option( 'colorshop_shop_page_display', 'subcategories' );
	} else {
		update_option( 'colorshop_shop_page_display', 'both' );
	}
}

if ( get_option( 'colorshop_show_subcategories' ) == 'yes' ) {
	if ( get_option( 'colorshop_hide_products_when_showing_subcategories' ) == 'yes' ) {
		update_option( 'colorshop_category_archive_display', 'subcategories' );
	} else {
		update_option( 'colorshop_category_archive_display', 'both' );
	}
}

// Update tax rates
$loop = 0;
$tax_rates = get_option( 'colorshop_tax_rates' );
...
$local_tax_rates = get_option( 'colorshop_local_tax_rates' );
...
update_option( 'colorshop_tax_rates_backup', $tax_rates );
update_option( 'colorshop_local_tax_rates_backup', $local_tax_rates );
delete_option( 'colorshop_tax_rates' );
delete_option( 'colorshop_local_tax_rates' );
...
// Reverse with UPDATE `wpcs_postmeta` SET meta_key = '_order_items' WHERE meta_key = '_order_items_old'
...
foreach ( $order_item_rows as $order_item_row ) {
	...
	foreach ( $order_items as $order_item ) {
		...
		$item_id = colorshop_add_order_item( $order_item_row->post_id, array(
	 	...
	 	// Add line item meta
	 	if ( $item_id ) {
		 	colorshop_add_order_item_meta( $item_id, '_qty', absint( $order_item['qty'] ) );
		 	colorshop_add_order_item_meta( $item_id, '_tax_class', $order_item['tax_class'] );
		 	colorshop_add_order_item_meta( $item_id, '_product_id', $order_item['id'] );
		 	colorshop_add_order_item_meta( $item_id, '_variation_id', $order_item['variation_id'] );
		 	colorshop_add_order_item_meta( $item_id, '_line_subtotal', colorshop_format_decimal( $order_item['line_subtotal'] ) );
		 	colorshop_add_order_item_meta( $item_id, '_line_subtotal_tax', colorshop_format_decimal( $order_item['line_subtotal_tax'] ) );
		 	colorshop_add_order_item_meta( $item_id, '_line_total', colorshop_format_decimal( $order_item['line_total'] ) );
		 	colorshop_add_order_item_meta( $item_id, '_line_tax', colorshop_format_decimal( $order_item['line_tax'] ) );
			...
			if ( sizeof( $meta_rows ) > 0 ) {
				$wpdb->query( $wpdb->prepare( "
					INSERT INTO {$wpdb->prefix}colorshop_order_itemmeta ( order_item_id, meta_key, meta_value )
...
// Reverse with UPDATE `wpcs_postmeta` SET meta_key = '_order_taxes' WHERE meta_key = '_order_taxes_old'
...
foreach ( $order_tax_rows as $order_tax_row ) {
	...
	if ( $order_taxes ) {
		foreach( $order_taxes as $order_tax ) {
			...
			$item_id = colorshop_add_order_item( $order_tax_row->post_id, array(
		 	...
		 	// Add line item meta
		 	if ( $item_id ) {
			 	colorshop_add_order_item_meta( $item_id, 'compound', absint( isset( $order_tax['compound'] ) ? $order_tax['compound'] : 0 ) );
			 	colorshop_add_order_item_meta( $item_id, 'tax_amount', colorshop_clean( $order_tax['cart_tax'] ) );
			 	colorshop_add_order_item_meta( $item_id, 'shipping_tax_amount', colorshop_clean( $order_tax['shipping_tax'] ) );
			}
...
foreach ( array( 'catalog', 'single', 'thumbnail' ) as $value ) {
	...
	$old_settings = array_filter( array(
		'width' => get_option( 'colorshop_' . $value . '_image_width' ),
		'height' => get_option( 'colorshop_' . $value . '_image_height' ),
		'crop' => get_option( 'colorshop_' . $value . '_image_crop' )
	) );

	if ( ! empty(  $old_settings  ) && update_option( 'shop_' . $value . '_image_size', $old_settings ) ){

		delete_option( 'colorshop_' . $value . '_image_width' );
		delete_option( 'colorshop_' . $value . '_image_height' );
		delete_option( 'colorshop_' . $value . '_image_crop' );
