...
function variable_product_type_options_tab() {
	?>
	<li class="variations_tab show_if_variable variation_options"><a href="#variable_product_options" title="<?php _e( 'Variations for variable products are defined here.', 'colorshop' ); ?>"><?php _e( 'Variations', 'colorshop' ); ?></a></li>
	<?php
}

add_action( 'colorshop_product_write_panel_tabs', 'variable_product_type_options_tab' );
...
function variable_product_type_options() {
	...
	?>
	<div id="variable_product_options" class="panel cs-metaboxes-wrapper"><div id="variable_product_options_inner">

		<?php if ( ! $variation_attribute_found ) : ?>

			<div id="message" class="inline colorshop-message">
				<div class="squeezer">
					<h4><?php _e( 'Before adding variations, add and save some attributes on the <strong>Attributes</strong> tab.', 'colorshop' ); ?></h4>

					<p class="submit"><a class="button-primary" href="http://colorvila.com/docs/plugins/colorshop/user-guide/admin/variable-product/" target="_blank"><?php _e( 'Learn more', 'colorshop' ); ?></a></p>
				</div>
			</div>
		...
		<?php else : ?>
			...
			<p class="toolbar">
				...
				<strong><?php _e( 'Bulk edit:', 'colorshop' ); ?></strong>
				<select id="field_to_edit">
					<option value="toggle_enabled"><?php _e( 'Toggle &quot;Enabled&quot;', 'colorshop' ); ?></option>
					<option value="toggle_downloadable"><?php _e( 'Toggle &quot;Downloadable&quot;', 'colorshop' ); ?></option>
					<option value="toggle_virtual"><?php _e( 'Toggle &quot;Virtual&quot;', 'colorshop' ); ?></option>
					<option value="delete_all"><?php _e( 'Delete all variations', 'colorshop' ); ?></option>
					<option value="variable_regular_price"><?php _e( 'Prices', 'colorshop' ); ?></option>
					<option value="variable_sale_price"><?php _e( 'Sale prices', 'colorshop' ); ?></option>
					<option value="variable_stock"><?php _e( 'Stock', 'colorshop' ); ?></option>
					<option value="variable_weight"><?php _e( 'Weight', 'colorshop' ); ?></option>
					<option value="variable_length"><?php _e( 'Length', 'colorshop' ); ?></option>
					<option value="variable_width"><?php _e( 'Width', 'colorshop' ); ?></option>
					<option value="variable_height"><?php _e( 'Height', 'colorshop' ); ?></option>
					<option value="variable_file_paths" rel="textarea"><?php _e( 'File Path', 'colorshop' ); ?></option>
					<option value="variable_download_limit"><?php _e( 'Download limit', 'colorshop' ); ?></option>
					<option value="variable_download_expiry"><?php _e( 'Download Expiry', 'colorshop' ); ?></option>
					<?php do_action( 'colorshop_variable_product_bulk_edit_actions' ); ?>
				</select>
				<a class="button bulk_edit"><?php _e( 'Go', 'colorshop' ); ?></a>
			</p>

			<div class="colorshop_variations cs-metaboxes">
				<?php
				// Get parent data
				$parent_data = array(
					'id'		=> $post->ID,
			...
			<p class="toolbar">
				...
				<button type="button" class="button button-primary add_variation" <?php disabled( $variation_attribute_found, false ); ?>><?php _e( 'Add Variation', 'colorshop' ); ?></button>

				<button type="button" class="button link_all_variations" <?php disabled( $variation_attribute_found, false ); ?>><?php _e( 'Link all variations', 'colorshop' ); ?></button>

				<strong><?php _e( 'Default selections:', 'colorshop' ); ?></strong>
				<?php
					...
					foreach ( $attributes as $attribute ) {
						...
						echo '<select name="default_attribute_' . sanitize_title( $attribute['name'] ) . '"><option value="">' . __( 'No default', 'colorshop' ) . ' ' . esc_html( $colorshop->attribute_label( $attribute['name'] ) ) . '&hellip;</option>';
						...
						if ( $attribute['is_taxonomy'] ) {
							...
							foreach ( $post_terms as $term )
								echo '<option ' . selected( $variation_selected_value, $term->slug, false ) . ' value="' . esc_attr( $term->slug ) . '">' . apply_filters( 'colorshop_variation_option_name', esc_html( $term->name ) ) . '</option>';
							...
							foreach ( $options as $option )
								echo '<option ' . selected( sanitize_title( $variation_selected_value ), sanitize_title( $option ), false ) . ' value="' . esc_attr( sanitize_title( $option ) ) . '">' . esc_html( apply_filters( 'colorshop_variation_option_name', $option ) )  . '</option>';
	...
	jQuery(function(){
		...
		jQuery('#variable_product_options').on('click', 'button.add_variation', function(){
			...
			jQuery('.colorshop_variations').block({ message: null, overlayCSS: { background: '#fff url(<?php echo $colorshop->plugin_url(); ?>/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

			var loop = jQuery('.colorshop_variation').size();

			var data = {
				action: 'colorshop_add_variation',
			...
			jQuery.post('<?php echo admin_url('admin-ajax.php'); ?>', data, function(response) {
				...
				jQuery('.colorshop_variations').append( response );
				...
				jQuery('.colorshop_variations').unblock();
				jQuery('#variable_product_options').trigger('colorshop_variations_added');
			});
		...
		jQuery('#variable_product_options').on('click', 'button.link_all_variations', function(){
			...
			var answer = confirm('<?php _e( 'Are you sure you want to link all variations? This will create a new variation for each and every possible combination of variation attributes (max 50 per run).', 'colorshop' ); ?>');

			if (answer) {

				jQuery('#variable_product_options').block({ message: null, overlayCSS: { background: '#fff url(<?php echo $colorshop->plugin_url(); ?>/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

				var data = {
					action: 'colorshop_link_all_variations',
					post_id: <?php echo $post->ID; ?>,
				...
				jQuery.post('<?php echo admin_url('admin-ajax.php'); ?>', data, function(response) {
					...
					if (count==1) {
						alert( count + ' <?php _e( "variation added", 'colorshop' ); ?>');
					} else if (count==0 || count>1) {
						alert( count + ' <?php _e( "variations added", 'colorshop' ); ?>');
					} else {
						alert('<?php _e( "No variations added", 'colorshop' ); ?>');
					}
		...
		jQuery('#variable_product_options').on('click', 'button.remove_variation', function(e){
			e.preventDefault();
			var answer = confirm('<?php _e( 'Are you sure you want to remove this variation?', 'colorshop' ); ?>');
			if (answer){
				...
				if (variation>0) {
					...
					jQuery(el).block({ message: null, overlayCSS: { background: '#fff url(<?php echo $colorshop->plugin_url(); ?>/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

					var data = {
						action: 'colorshop_remove_variation',
		...
		jQuery('.cs-metaboxes-wrapper').on('click', 'a.bulk_edit', function(event){
			...
			if ( field_to_edit == 'toggle_enabled' ) {
				...
				var answer = confirm('<?php _e( 'Are you sure you want to delete all variations? This cannot be undone.', 'colorshop' ); ?>');
				if (answer){

					var answer = confirm('<?php _e( 'Last warning, are you sure?', 'colorshop' ); ?>');

					if (answer) {

						var variation_ids = [];

						jQuery('.colorshop_variations .colorshop_variation').block({ message: null, overlayCSS: { background: '#fff url(<?php echo $colorshop->plugin_url(); ?>/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });

						jQuery('.colorshop_variations .colorshop_variation .remove_variation').each(function(){
						...
						var data = {
							action: 'colorshop_remove_variations',
							variation_ids: variation_ids,
							security: '<?php echo wp_create_nonce("delete-variations"); ?>'
						};

						jQuery.post('<?php echo admin_url('admin-ajax.php'); ?>', data, function(response) {
							jQuery('.colorshop_variations .colorshop_variation').fadeOut('300', function(){
								jQuery('.colorshop_variations .colorshop_variation').remove();
							});
						});
				...
				var value = prompt("<?php _e( 'Enter a value', 'colorshop' ); ?>");
		...
		jQuery('#variable_product_options').on('change', 'input.variable_is_downloadable', function(){
			...
			jQuery(this).closest('.colorshop_variation').find('.show_if_variation_downloadable').hide();

			if (jQuery(this).is(':checked')) {
				jQuery(this).closest('.colorshop_variation').find('.show_if_variation_downloadable').show();
			}
		...
		jQuery('#variable_product_options').on('change', 'input.variable_is_virtual', function(){
			...
			jQuery(this).closest('.colorshop_variation').find('.hide_if_variation_virtual').show();

			if (jQuery(this).is(':checked')) {
				jQuery(this).closest('.colorshop_variation').find('.hide_if_variation_virtual').hide();
			}
		...
		// Ordering
		$('#variable_product_options').on( 'colorshop_variations_added', function() {
			$('.colorshop_variations').sortable( variation_sortable_options );
		} );

		$('.colorshop_variations').sortable( variation_sortable_options );

		function variation_row_indexes() {
			$('.colorshop_variations .colorshop_variation').each(function(index, el){
				$('.variation_menu_order', el).val( parseInt( $(el).index('.colorshop_variations .colorshop_variation') ) );
			});
		};
		...
		jQuery('#variable_product_options').on('click', '.upload_image_button', function( event ) {
			...
			if ( $button.is('.remove') ) {
				...
				setting_variation_image.find( 'img' ).attr( 'src', '<?php echo colorshop_placeholder_img_src(); ?>' );
				...
				variable_image_frame = wp.media.frames.variable_image = wp.media({
					...
					button: {
						text: '<?php _e( 'Set variation image', 'colorshop' ); ?>'
					}
				});
	...
	$colorshop->add_inline_js( $javascript );
}

add_action('colorshop_product_write_panels', 'variable_product_type_options');
...
function variable_product_type_selector( $types, $product_type ) {
	$types['variable'] = __( 'Variable product', 'colorshop' );
	return $types;
}
...
function process_product_meta_variable( $post_id ) {
	...
	if ( isset( $_POST['variable_sku'] ) ) {
		...
		for ( $i = 0; $i <= $max_loop; $i ++ ) {
			...
			$variation_post_title = sprintf( __( 'Variation #%s of %s', 'colorshop' ), absint( $variation_id ), esc_html( get_the_title( $post_id ) ) );
			...
			if ( ! $variation_id ) {
				...
				do_action( 'colorshop_update_product_variation', $variation_id );

			}

			// Update post meta
			update_post_meta( $variation_id, '_sku', colorshop_clean( $variable_sku[ $i ] ) );
			update_post_meta( $variation_id, '_weight', colorshop_clean( $variable_weight[ $i ] ) );

			update_post_meta( $variation_id, '_length', colorshop_clean( $variable_length[ $i ] ) );
			update_post_meta( $variation_id, '_width', colorshop_clean( $variable_width[ $i ] ) );
			update_post_meta( $variation_id, '_height', colorshop_clean( $variable_height[ $i ] ) );

			update_post_meta( $variation_id, '_stock', colorshop_clean( $variable_stock[ $i ] ) );
			update_post_meta( $variation_id, '_thumbnail_id', absint( $upload_image_id[ $i ] ) );

			update_post_meta( $variation_id, '_virtual', colorshop_clean( $is_virtual ) );
			update_post_meta( $variation_id, '_downloadable', colorshop_clean( $is_downloadable ) );

			// Price handling
			$regular_price 	= colorshop_clean( $variable_regular_price[ $i ] );
			$sale_price 	= colorshop_clean( $variable_sale_price[ $i ] );
			$date_from 		= colorshop_clean( $variable_sale_price_dates_from[ $i ] );
			$date_to		= colorshop_clean( $variable_sale_price_dates_to[ $i ] );
			...
			if ( $variable_tax_class[ $i ] !== 'parent' )
				update_post_meta( $variation_id, '_tax_class', colorshop_clean( $variable_tax_class[ $i ] ) );
			else
				delete_post_meta( $variation_id, '_tax_class' );

			if ( $is_downloadable == 'yes' ) {
				update_post_meta( $variation_id, '_download_limit', colorshop_clean( $variable_download_limit[ $i ] ) );
				update_post_meta( $variation_id, '_download_expiry', colorshop_clean( $variable_download_expiry[ $i ] ) );
				...
				do_action( 'colorshop_process_product_file_download_paths', $post_id, $variation_id, $_file_paths );
			...
			do_action( 'colorshop_save_product_variation', $variation_id );
...
add_action( 'colorshop_process_product_meta_variable', 'process_product_meta_variable' );