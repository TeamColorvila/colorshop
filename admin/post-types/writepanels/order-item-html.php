...
<tr class="item <?php if ( ! empty( $class ) ) echo $class; ?>" data-order_item_id="<?php echo $item_id; ?>">
	...
	<td class="thumb">
		<a href="<?php echo esc_url( admin_url( 'post.php?post=' . absint( $_product->id ) . '&action=edit' ) ); ?>" class="tips" data-tip="<?php
			...
			echo '<strong>' . __( 'Product ID:', 'colorshop' ) . '</strong> ' . absint( $item['product_id'] );

			if ( $item['variation_id'] )
				echo '<br/><strong>' . __( 'Variation ID:', 'colorshop' ) . '</strong> ' . absint( $item['variation_id'] );

			if ( $_product->get_sku() )
				echo '<br/><strong>' . __( 'Product SKU:', 'colorshop' ).'</strong> ' . esc_html( $_product->get_sku() );
	...
	<td class="name">
		...
		<?php
			if ( isset( $_product->variation_data ) )
				echo '<br/>' . colorshop_get_formatted_variation( $_product->variation_data, true );
		?>
		<table class="meta" cellspacing="0">
			<tfoot>
				<tr>
					<td colspan="4"><button class="add_order_item_meta button"><?php _e( 'Add&nbsp;meta', 'colorshop' ); ?></button></td>
				</tr>
			</tfoot>
			...
			<?php
				if ( $metadata = $order->has_meta( $item_id )) {
					foreach ( $metadata as $meta ) {
						...
						if ( in_array( $meta['meta_key'], apply_filters( 'colorshop_hidden_order_itemmeta', array(
							'_qty',
							'_tax_class',
							'_product_id',
							'_variation_id',
	...
	<?php do_action( 'colorshop_admin_order_item_values', $_product, $item, absint( $item_id ) ); ?>
	...
	<td class="quantity" width="1%">
		<input type="number" step="<?php echo apply_filters( 'colorshop_quantity_input_step', '1', $_product ); ?>" min="0" autocomplete="off" name="order_item_qty[<?php echo absint( $item_id ); ?>]" placeholder="0" value="<?php echo esc_attr( $item['qty'] ); ?>" size="4" class="quantity" />
	</td>

	<td class="line_cost" width="1%">
		<label><?php _e( 'Total', 'colorshop' ); ?>: <input type="number" step="any" min="0" name="line_total[<?php echo absint( $item_id ); ?>]" placeholder="0.00" value="<?php if ( isset( $item['line_total'] ) ) echo esc_attr( $item['line_total'] ); ?>" class="line_total" /></label>

		<span class="subtotal"><label><?php _e( 'Subtotal', 'colorshop' ); ?>: <input type="number" step="any" min="0" name="line_subtotal[<?php echo absint( $item_id ); ?>]" placeholder="0.00" value="<?php if ( isset( $item['line_subtotal'] ) ) echo esc_attr( $item['line_subtotal'] ); ?>" class="line_subtotal" /></label></span>
	</td>
