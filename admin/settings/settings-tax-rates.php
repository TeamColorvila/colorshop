 ...
 * @return void
 */
function colorshop_tax_rates_setting() {
	global $colorshop, $current_section, $wpdb;
	...
	foreach( $tax_classes as $class )
		if ( sanitize_title( $class ) == $current_section )
			$current_class = $class;
	?>
	<h3><?php printf( __( 'Tax Rates for the "%s" Class', 'colorshop' ), $current_class ? esc_html( $current_class ) : __( 'Standard', 'colorshop' ) ); ?></h3>
	<p><?php printf( __( 'Define tax rates for countries and states below. <a href="%s">See here</a> for available alpha-2 country codes.', 'colorshop' ), 'http://en.wikipedia.org/wiki/ISO_3166-1#Current_codes' ); ?></p>
	<table class="cs_tax_rates widefat">
		<thead>
			<tr>
				<th class="sort">&nbsp;</th>

				<th width="8%"><?php _e( 'Country&nbsp;Code', 'colorshop' ); ?>&nbsp;<span class="tips" data-tip="<?php _e('A 2 digit country code, e.g. US. Leave blank to apply to all.', 'colorshop'); ?>">[?]</span></th>

				<th width="8%"><?php _e( 'State&nbsp;Code', 'colorshop' ); ?>&nbsp;<span class="tips" data-tip="<?php _e('A 2 digit state code, e.g. AL. Leave blank to apply to all.', 'colorshop'); ?>">[?]</span></th>

				<th><?php _e( 'ZIP/Postcode', 'colorshop' ); ?>&nbsp;<span class="tips" data-tip="<?php _e('Postcode for this rule. Semi-colon (;) separate multiple values. Leave blank to apply to all areas. Wildcards (*) can be used. Ranges for numeric postcodes (e.g. 12345-12350) will be expanded into individual postcodes.', 'colorshop'); ?>">[?]</span></th>

				<th><?php _e( 'City', 'colorshop' ); ?>&nbsp;<span class="tips" data-tip="<?php _e('Cities for this rule. Semi-colon (;) separate multiple values. Leave blank to apply to all cities.', 'colorshop'); ?>">[?]</span></th>

				<th width="8%"><?php _e( 'Rate&nbsp;%', 'colorshop' ); ?>&nbsp;<span class="tips" data-tip="<?php _e( 'Enter a tax rate (percentage) to 4 decimal places.', 'colorshop' ); ?>">[?]</span></th>

				<th width="8%"><?php _e( 'Tax&nbsp;Name', 'colorshop' ); ?>&nbsp;<span class="tips" data-tip="<?php _e('Enter a name for this tax rate.', 'colorshop'); ?>">[?]</span></th>

				<th width="8%"><?php _e( 'Priority', 'colorshop' ); ?>&nbsp;<span class="tips" data-tip="<?php _e('Choose a priority for this tax rate. Only 1 matching rate per priority will be used. To define multiple tax rates for a single area you need to specify a different priority per rate.', 'colorshop'); ?>">[?]</span></th>

				<th width="8%"><?php _e( 'Compound', 'colorshop' ); ?>&nbsp;<span class="tips" data-tip="<?php _e('Choose whether or not this is a compound rate. Compound tax rates are applied on top of other tax rates.', 'colorshop'); ?>">[?]</span></th>

				<th width="8%"><?php _e( 'Shipping', 'colorshop' ); ?>&nbsp;<span class="tips" data-tip="<?php _e('Choose whether or not this tax rate also gets applied to shipping.', 'colorshop'); ?>">[?]</span></th>
		...
		<tfoot>
			<tr>
				<th colspan="10">
					<a href="#" class="button plus insert"><?php _e( 'Insert row', 'colorshop' ); ?></a>
					<a href="#" class="button minus remove"><?php _e( 'Remove selected row(s)', 'colorshop' ); ?></a>

					<a href="#" download="tax_rates.csv" class="button export"><?php _e( 'Export CSV', 'colorshop' ); ?></a>
					<a href="<?php echo admin_url( 'admin.php?import=colorshop_tax_rate_csv' ); ?>" class="button import"><?php _e( 'Import CSV', 'colorshop' ); ?></a>
				</th>
			</tr>
		</tfoot>
		<tbody id="rates">
			<?php
				$rates = $wpdb->get_results( $wpdb->prepare(
					"SELECT * FROM {$wpdb->prefix}colorshop_tax_rates
					...
					<tr>
						...
						<td class="postcode">
							<input type="text" value="<?php
								$locations = $wpdb->get_col( $wpdb->prepare( "SELECT location_code FROM {$wpdb->prefix}colorshop_tax_rate_locations WHERE location_type='postcode' AND tax_rate_id = %d ORDER BY location_code", $rate->tax_rate_id ) );
						...
						<td class="city">
							<input type="text" value="<?php
								$locations = $wpdb->get_col( $wpdb->prepare( "SELECT location_code FROM {$wpdb->prefix}colorshop_tax_rate_locations WHERE location_type='city' AND tax_rate_id = %d ORDER BY location_code", $rate->tax_rate_id ) );
								echo esc_attr( implode( '; ', $locations ) );
	...
	<script type="text/javascript">
		jQuery( function() {
			jQuery('.cs_tax_rates tbody').sortable({
				items:'tr',
				cursor:'move',
				axis:'y',
			...
			jQuery('.cs_tax_rates .remove').click(function() {
				var $tbody = jQuery('.cs_tax_rates').find('tbody');
				if ( $tbody.find('tr.current').size() > 0 ) {
					...
					alert('<?php _e( 'No row(s) selected', 'colorshop' ); ?>');
				}
				return false;
			});
			...
			jQuery('.cs_tax_rates .export').click(function() {

				var csv_data = "data:application/csv;charset=utf-8,<?php _e( 'Country Code', 'colorshop' ); ?>,<?php _e( 'State Code', 'colorshop' ); ?>,<?php _e( 'ZIP/Postcode', 'colorshop' ); ?>,<?php _e( 'City', 'colorshop' ); ?>,<?php _e( 'Rate %', 'colorshop' ); ?>,<?php _e( 'Tax Name', 'colorshop' ); ?>,<?php _e( 'Priority', 'colorshop' ); ?>,<?php _e( 'Compound', 'colorshop' ); ?>,<?php _e( 'Shipping', 'colorshop' ); ?>,<?php _e( 'Tax Class', 'colorshop' ); ?>\n";
			...
			jQuery('.cs_tax_rates .insert').click(function() {
				var $tbody = jQuery('.cs_tax_rates').find('tbody');
			...
			jQuery('.cs_tax_rates td.postcode, .cs_tax_rates td.city').find('input').change(function() {
				jQuery(this).attr( 'name', jQuery(this).attr( 'data-name' ) );
			});

			var availableCountries = [<?php
				$countries = array();
				foreach ( $colorshop->countries->get_allowed_countries() as $value => $label )
			...
			var availableStates = [<?php
				$countries = array();
				foreach ( $colorshop->countries->get_allowed_country_states() as $value => $label )
...
/**
 * colorshop_tax_rates_setting_save function.
 *
 * @access public
 * @return void
 */
function colorshop_tax_rates_setting_save() {
	...
	foreach ( $tax_rate_country as $key => $value ) {
		...
		if ( $key == 'new' ) {
			...
			foreach ( $value as $new_key => $new_value ) {
				...
				// Sanitize + format
				$country  = strtoupper( colorshop_clean( $tax_rate_country[ $key ][ $new_key ] ) );
				$state    = strtoupper( colorshop_clean( $tax_rate_state[ $key ][ $new_key ] ) );
				$postcode = colorshop_clean( $tax_rate_postcode[ $key ][ $new_key ] );
				$city     = colorshop_clean( $tax_rate_city[ $key ][ $new_key ] );
				$rate     = number_format( colorshop_clean( $tax_rate[ $key ][ $new_key ] ), 4, '.', '' );
				$name     = colorshop_clean( $tax_rate_name[ $key ][ $new_key ] );
				$priority = absint( colorshop_clean( $tax_rate_priority[ $key ][ $new_key ] ) );
				$compound = isset( $tax_rate_compound[ $key ][ $new_key ] ) ? 1 : 0;
				$shipping = isset( $tax_rate_shipping[ $key ][ $new_key ] ) ? 1 : 0;

				if ( ! $name )
					$name = __( 'Tax', 'colorshop' );
				...
				if ( ! empty( $postcode ) ) {
					...
					$wpdb->query( "INSERT INTO {$wpdb->prefix}colorshop_tax_rate_locations ( location_code, tax_rate_id, location_type ) VALUES " . implode( ',', $postcode_query ) );
				}
		...
		// ...whereas the others are updated
			...
			if ( $_POST['remove_tax_rate'][ $key ] == 1 ) {
				$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}colorshop_tax_rate_locations WHERE tax_rate_id = %d;", $tax_rate_id ) );
				$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}colorshop_tax_rates WHERE tax_rate_id = %d;", $tax_rate_id ) );
				continue;
			}

			// Sanitize + format
			$country  = strtoupper( colorshop_clean( $tax_rate_country[ $key ] ) );
			$state    = strtoupper( colorshop_clean( $tax_rate_state[ $key ] ) );
			$rate     = number_format( colorshop_clean( $tax_rate[ $key ] ), 4, '.', '' );
			$name     = colorshop_clean( $tax_rate_name[ $key ] );
			$priority = absint( colorshop_clean( $tax_rate_priority[ $key ] ) );
			...
			if ( isset( $tax_rate_postcode[ $key ] ) ) {
				// Delete old
				$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}colorshop_tax_rate_locations WHERE tax_rate_id = %d AND location_type = 'postcode';", $tax_rate_id ) );

				// Add changed
				$postcode  = colorshop_clean( $tax_rate_postcode[ $key ] );
				$postcodes = explode( ';', $postcode );
				$postcodes = array_map( 'strtoupper', array_map( 'colorshop_clean', $postcodes ) );
			...
			if ( isset( $tax_rate_city[ $key ] ) ) {
				// Delete old
				$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}colorshop_tax_rate_locations WHERE tax_rate_id = %d AND location_type = 'city';", $tax_rate_id ) );

				// Add changed
				$city   = colorshop_clean( $tax_rate_city[ $key ] );
				$cities = explode( ';', $city );
				$cities = array_map( 'strtoupper', array_map( 'colorshop_clean', $cities ) );
				foreach( $cities as $city ) {
					if ( $city ) {
