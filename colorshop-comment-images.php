<?php
/**
 * ColorShop Comment Image
 *
 * colorshop comment image class
 *
 * @author 		ColorVila
 * @category 	Core
 * @package 	ColorShop
...
class Comment_Image {
	...
	function __construct() {
		...
		if( $this->can_save_files() ) {
			...
			add_filter( 'manage_comments_columns', array( $this, 'post_has_comment_images' ) );
			add_filter( 'manage_comments_custom_column', array( $this, 'post_comment_images' ), 20, 2 );
	 ...
	 *---------------------------------------------*/
	 ...
	 public function comment_image( $column_name, $comment_id ) {
		 ...
		 if( 'comment-image' == strtolower( $column_name ) ) {
			 ...
			 if( 0 != ( $comment_image_datas = get_comment_meta( $comment_id, 'comment_image', true ) ) ) {
				 foreach ($comment_image_datas as $comment_image_data) {
				 	$image_url = $comment_image_data;
				 	$html .= '<img src="' . $image_url . '" style="margin-right:5px; width:60px; height:80px;" />';
				 }
	...
	function save_comment_image( $comment_id ) {
		...
		$comment_image_file = array();
		if ( ! empty($_POST['img1']) )
			$comment_image_file[] = $_POST['img1'];

		if ( ! empty($_POST['img2']) )
			$comment_image_file[] = $_POST['img2'];
		
		if ( ! empty($_POST['img3']) )
			$comment_image_file[] = $_POST['img3'];
		
		if ( ! empty($_POST['img4']) )
			$comment_image_file[] = $_POST['img4'];
		
		if ( ! empty($_POST['img5']) )
			$comment_image_file[] = $_POST['img5'];
		...
		if( count( $comments ) > 0 ) {
			...
			foreach( $comments as $comment ) {
				...
				if( true == get_comment_meta( $comment->comment_ID, 'comment_image' ) ) {
					...
					$comment_images = get_comment_meta( $comment->comment_ID, 'comment_image', true );
					
					$comment->comment_content .= '<div class="cs-comment-image">';
					$random = uniqid();
					foreach ($comment_images as $comment_image) {
						//$comment->comment_content .= '<div class="comment-image">';
						$comment->comment_content .= '<a href="' . $this->get_big_image_path($comment_image) . '" rel="prettyPhoto[pp_gal_'. $random .']">';
						$comment->comment_content .= '<img src="' . $comment_image . '" alt="" />';
						$count++;
						$comment->comment_content .= '</a>';
						//$comment->comment_content .= '</div><!-- /.comment-image -->';
					}
