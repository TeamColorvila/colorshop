				...
				<tr>
					<td>
						...
						<?php elseif ( ! $quantites_required ) : ?>
							...
							<a href="<?php echo esc_url( $child_product['product']->add_to_cart_url() ); ?>" rel="nofollow" class="single_add_to_cart_button button alt"><?php echo apply_filters( 'single_add_to_cart_text', __( 'Add to cart', 'colorshop' ), $child_product['product']->product_type ); ?></a>

						<?php else : ?>

							<?php colorshop_quantity_input( array( 'input_name' => 'quantity['.$child_product['product']->id.']', 'input_value' => '0' ) ); ?>
					...
					<?php do_action ( 'colorshop_grouped_product_list_before_price', $child_product['product'] ); ?>

					<td class="price"><?php echo $child_product['product']->get_price_html(); ?>
					<?php echo apply_filters( 'colorshop_stock_html', '<small class="stock '.$child_product['availability']['class'].'">'.$child_product['availability']['availability'].'</small>', $child_product['availability']['availability'] ); ?>
					</td>
				</tr>
			<?php endforeach; ?>
		</tbody>
