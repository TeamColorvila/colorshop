	...
	<table class="variations" cellspacing="0">
		<tbody>
			<?php $loop = 0; foreach ( $attributes as $name => $options ) : $loop++; ?>
				<tr>
					<td class="label"><label for="<?php echo sanitize_title($name); ?>"><?php echo $colorshop->attribute_label( $name ); ?></label></td>
					<td class="value"><select id="<?php echo esc_attr( sanitize_title($name) ); ?>" name="attribute_<?php echo sanitize_title($name); ?>">
						<option value=""><?php echo __( 'Choose an option', 'colorshop' ) ?>&hellip;</option>
						<?php
							if ( is_array( $options ) ) {
								...
								if ( taxonomy_exists( sanitize_title( $name ) ) ) {
									...
									$orderby = $colorshop->attribute_orderby( $name );
									...
									foreach ( $terms as $term ) {
										...
										echo '<option value="' . esc_attr( $term->slug ) . '" ' . selected( $selected_value, $term->slug, false ) . '>' . apply_filters( 'colorshop_variation_option_name', $term->name ) . '</option>';
									}
								} else {

									foreach ( $options as $option ) {
										echo '<option value="' . esc_attr( sanitize_title( $option ) ) . '" ' . selected( sanitize_title( $selected_value ), sanitize_title( $option ), false ) . '>' . esc_html( apply_filters( 'colorshop_variation_option_name', $option ) ) . '</option>';
									}
						...
						if ( sizeof($attributes) == $loop )
							echo '<a class="reset_variations" href="#reset">' . __( 'Clear selection', 'colorshop' ) . '</a>';
					?></td>
				</tr>
	        <?php endforeach;?>
		</tbody>
