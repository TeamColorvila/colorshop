 ...
 *
 * @author ColorVila
 * @package ColorShop/Templates/Emails/HTML
 * @version 1.0.0
 */
...
<p><?php printf( __( 'You have received an order from %s. Their order is as follows:', 'colorshop' ), $order->billing_first_name . ' ' . $order->billing_last_name ); ?></p>

<?php do_action( 'colorshop_email_before_order_table', $order, true ); ?>

<h2><?php printf( __( 'Order: %s', 'colorshop'), $order->get_order_number() ); ?> (<?php printf( '<time datetime="%s">%s</time>', date_i18n( 'c', strtotime( $order->order_date ) ), date_i18n( colorshop_date_format(), strtotime( $order->order_date ) ) ); ?>)</h2>
...
<?php do_action('colorshop_email_after_order_table', $order, true); ?>

<?php do_action( 'colorshop_email_order_meta', $order, true ); ?>
