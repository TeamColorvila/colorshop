 ...
 *
 * @class 		CS_Shipping_Flat_Rate
 * @version		1.0.0
 * @package		ColorShop/Classes/Shipping
 * @author 		ColorVila
 */
class CS_Shipping_Flat_Rate extends CS_Shipping_Method {
	...
	function __construct() {
        $this->id 						= 'flat_rate';
        $this->method_title 			= __( 'Flat Rate', 'colorshop' );
		$this->flat_rate_option 		= 'colorshop_flat_rates';
		$this->method_description 	    = __( 'Flat rates let you define a standard rate per item, or per order.', 'colorshop' );
		...
		add_filter( 'colorshop_settings_api_sanitized_fields_' . $this->id, array( $this, 'save_default_costs' ) );
    ...
    function init_form_fields() {
    	...
    	$this->form_fields = array(
			...
			'title' => array(
							...
							'default'		=> __( 'Flat Rate', 'colorshop' ),
							'desc_tip'      => true
						),
			'availability' => array(
			...
			'cost_per_order' => array(
							'title' 		=> __( 'Cost per order', 'colorshop' ),
							'type' 			=> 'number',
							'custom_attributes' => array(
								'step'	=> 'any',
								'min'	=> '0'
							),
							'description'	=> __( 'Enter a cost (excluding tax) per order, e.g. 5.00. Leave blank to disable.', 'colorshop' ),
							'default'		=> '',
							'desc_tip'      => true,
							'placeholder'	=> '0.00'
						),
			'options' => array(
							'title' 		=> __( 'Additional Rates', 'colorshop' ),
							'type' 			=> 'textarea',
							'description'	=> __( 'Optional extra shipping options with additional costs (one per line): Option Name | Additional Cost | Per Cost Type (order, class, or item) Example: <code>Priority Mail | 6.95 | order</code>.', 'colorshop' ),
							'default'		=> '',
							'desc_tip'      => true,
							'placeholder'	=> __( 'Option Name | Additional Cost | Per Cost Type (order, class, or item)', 'colorshop' )
						),
			'additional_costs' => array(
							'title'         => __( 'Additional Costs', 'colorshop' ),
							'type'          => 'title',
							'description'   => __( 'Additional costs can be added below - these will all be added to the per-order cost above.', 'colorshop' )
						),
			'type' => array(
							'title' 		=> __( 'Costs Added...', 'colorshop' ),
							'type' 			=> 'select',
							'default' 		=> 'order',
							'options' 		=> array(
    ...
    function generate_additional_costs_table_html() {
	    ...
	    ?>
	    <tr valign="top">
            <th scope="row" class="titledesc"><?php _e( 'Costs', 'colorshop' ); ?>:</th>
            <td class="forminp" id="<?php echo $this->id; ?>_flat_rates">
			    <table class="shippingrows widefat" cellspacing="0">
		    		<thead>
		    			<tr>
		    				<th class="check-column"><input type="checkbox"></th>
		    				<th class="shipping_class"><?php _e( 'Shipping Class', 'colorshop' ); ?></th>
			            	<th><?php _e( 'Cost', 'colorshop' ); ?> <a class="tips" data-tip="<?php _e( 'Cost, excluding tax.', 'colorshop' ); ?>">[?]</a></th>
			            	<th><?php _e( 'Handling Fee', 'colorshop' ); ?> <a class="tips" data-tip="<?php _e( 'Fee excluding tax. Enter an amount, e.g. 2.50, or a percentage, e.g. 5%.', 'colorshop' ); ?>">[?]</a></th>
		    			</tr>
		    		</thead>
		    		<tfoot>
		    			<tr>
		    				<th colspan="4"><a href="#" class="add button"><?php _e( '+ Add Cost', 'colorshop' ); ?></a> <a href="#" class="remove button"><?php _e( 'Delete selected costs', 'colorshop' ); ?></a></th>
		    			</tr>
		    		</tfoot>
		    		<tbody class="flat_rates">
		    			<tr>
		    				<td></td>
		    				<td class="flat_rate_class"><?php _e( 'Any class', 'colorshop' ); ?></td>
		    				<td><input type="number" step="any" min="0" value="<?php echo esc_attr( $this->cost ); ?>" name="default_cost" placeholder="<?php _e( 'N/A', 'colorshop' ); ?>" size="4" /></td>
				            <td><input type="text" value="<?php echo esc_attr( $this->fee ); ?>" name="default_fee" placeholder="<?php _e( 'N/A', 'colorshop' ); ?>" size="4" /></td>
		    			</tr>
		            	<?php
		            	$i = -1;
		            	if ( $this->flat_rates ) {
		            		foreach ( $this->flat_rates as $class => $rate ) {
		                		...
		                		if ( $colorshop->shipping->get_shipping_classes() ) {
			                		foreach ( $colorshop->shipping->get_shipping_classes() as $shipping_class ) {
			                			echo '<option value="'.$shipping_class->slug.'" '.selected($shipping_class->slug, $class, false).'>'.$shipping_class->name.'</option>';
			                		}
		                		} else {
		                			echo '<option value="">'.__( 'Select a class&hellip;', 'colorshop' ).'</option>';
		                		}

				                echo '</select>
				               		</td>
				                    <td><input type="number" step="any" min="0" value="' . esc_attr( $rate['cost'] ) . '" name="' . esc_attr( $this->id .'_cost[' . $i . ']' ) . '" placeholder="'.__( '0.00', 'colorshop' ).'" size="4" /></td>
				                    <td><input type="text" value="' . esc_attr( $rate['fee'] ) . '" name="' . esc_attr( $this->id .'_fee[' . $i . ']' ) . '" placeholder="'.__( '0.00', 'colorshop' ).'" size="4" /></td>
			                    </tr>';
		            		}
		            	}
		            	?>
		       	...
		       	<script type="text/javascript">
					jQuery(function() {
							...
							jQuery('<tr class="flat_rate">\
		            			...
		            			<td class="flat_rate_class">\
		            				<select name="<?php echo $this->id; ?>_class[' + size + ']" class="select">\
			               				<?php
			               				if ($colorshop->shipping->get_shipping_classes()) :
					                		foreach ($colorshop->shipping->get_shipping_classes() as $class) :
						...
						jQuery('#<?php echo $this->id; ?>_flat_rates').on( 'click', 'a.remove', function(){
							var answer = confirm("<?php _e( 'Delete the selected rates?', 'colorshop' ); ?>")
							if (answer) {
    ...
    function process_flat_rates() {
		...
		if ( isset( $_POST[ $this->id . '_class'] ) ) $flat_rate_class = array_map( 'colorshop_clean', $_POST[ $this->id . '_class'] );
		if ( isset( $_POST[ $this->id . '_cost'] ) )  $flat_rate_cost  = array_map( 'colorshop_clean', $_POST[ $this->id . '_cost'] );
		if ( isset( $_POST[ $this->id . '_fee'] ) )   $flat_rate_fee   = array_map( 'colorshop_clean', $_POST[ $this->id . '_fee'] );
    ...
    function save_default_costs( $fields ) {
	 	$default_cost = colorshop_clean( $_POST['default_cost'] );
	 	$default_fee  = colorshop_clean( $_POST['default_fee'] );
