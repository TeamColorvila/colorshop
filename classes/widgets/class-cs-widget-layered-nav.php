...
class CS_Widget_Layered_Nav extends WP_Widget {
	 ...
	 * @return void
	 */
	function CS_Widget_Layered_Nav() {

		/* Widget variable settings. */
		$this->color_widget_cssclass 		= 'colorshop widget_layered_nav';
		$this->color_widget_description	= __( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'colorshop' );
		$this->color_widget_idbase 		= 'colorshop_layered_nav';
		$this->color_widget_name 			= __( 'ColorShop Layered Nav', 'colorshop' );

		/* Widget settings. */
		$widget_ops = array( 'classname' => $this->color_widget_cssclass, 'description' => $this->color_widget_description );

		/* Create the widget. */
		$this->WP_Widget( 'colorshop_layered_nav', $this->color_widget_name, $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$taxonomy 		= $colorshop->attribute_taxonomy_name($instance['attribute']);
		...
		if ( count( $terms ) > 0 ) {
			...
			if ( $display_type == 'dropdown' ) {
				...
				if ( $current_tax && $taxonomy == $current_tax ) {
					...
					echo '<option value="">' . sprintf( __( 'Any %s', 'colorshop' ), $colorshop->attribute_label( $taxonomy ) ) .'</option>';
					...
					foreach ( $terms as $term ) {
						...
						$transient_name = 'cs_ln_count_' . md5( sanitize_key( $taxonomy ) . sanitize_key( $term->term_id ) );
						...
						if ( $query_type == 'and' ) {
							...
							$count = sizeof( array_intersect( $_products_in_term, $colorshop->query->filtered_product_ids ) );
						...
						// If this is an OR query, show all options so search can be expanded
							...
							$count = sizeof( array_intersect( $_products_in_term, $colorshop->query->unfiltered_product_ids ) );
				...
				foreach ( $terms as $term ) {
					...
					echo ( $count > 0 || $option_is_set ) ? '<a href="' . esc_url( apply_filters( 'colorshop_layered_nav_link', $link ) ) . '">' : '<span>';
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( empty( $new_instance['title'] ) )
			$new_instance['title'] = $colorshop->attribute_label( $new_instance['attribute'] );
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo $this->get_field_id( 'attribute' ); ?>"><?php _e( 'Attribute:', 'colorshop' ) ?></label>
		...
		<p><label for="<?php echo $this->get_field_id( 'display_type' ); ?>"><?php _e( 'Display Type:', 'colorshop' ) ?></label>
		<select id="<?php echo esc_attr( $this->get_field_id( 'display_type' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'display_type' ) ); ?>">
			<option value="list" <?php selected( $instance['display_type'], 'list' ); ?>><?php _e( 'List', 'colorshop' ); ?></option>
			<option value="dropdown" <?php selected( $instance['display_type'], 'dropdown' ); ?>><?php _e( 'Dropdown', 'colorshop' ); ?></option>
		</select></p>

		<p><label for="<?php echo $this->get_field_id( 'query_type' ); ?>"><?php _e( 'Query Type:', 'colorshop' ) ?></label>
		<select id="<?php echo esc_attr( $this->get_field_id( 'query_type' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'query_type' ) ); ?>">
			<option value="and" <?php selected( $instance['query_type'], 'and' ); ?>><?php _e( 'AND', 'colorshop' ); ?></option>
			<option value="or" <?php selected( $instance['query_type'], 'or' ); ?>><?php _e( 'OR', 'colorshop' ); ?></option>
		</select></p>
		<?php
	}
}