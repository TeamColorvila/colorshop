<?php
/**
 * Contains the query functions for ColorShop which alter the front-end post queries and loops.
 *
 * @class 		CS_Query
 * @version		1.0.0
...
class CS_Query {
	...
	public function pre_get_posts( $q ) {
		...
		if ( $q->is_home() && 'page' == get_option('show_on_front') && get_option('page_on_front') == colorshop_get_page_id('shop') ) {
		...
		if ( $q->is_page() && 'page' == get_option( 'show_on_front' ) && $q->get('page_id') == colorshop_get_page_id('shop') ) {
		...
		// We're on a shop page so queue the colorshop_get_products_in_view function
	...
	public function search_post_excerpt( $where = '' ) {
		...
		// If this is not a CS Query, do not modify the query
		if ( empty( $wp_the_query->query_vars['cs_query'] ) )
		    return $where;
	...
	public function wpseo_metadesc() {
		return wpseo_get_value( 'metadesc', colorshop_get_page_id('shop') );
	}
	...
	public function wpseo_metakey() {
		return wpseo_get_value( 'metakey', colorshop_get_page_id('shop') );
	}
	...
	public function product_query( $q ) {
		...
		do_action( 'colorshop_product_query', $q, $this );
	}
	...
	public function get_products_in_view() {
		...
		$transient_name = 'cs_uf_pid_' . md5( http_build_query( $current_wp_query ) );
	...
	public function get_catalog_ordering_args( $orderby = '', $order = '' ) {
		...
		if ( ! $orderby ) {
			$orderby_value = isset( $_GET['orderby'] ) ? colorshop_clean( $_GET['orderby'] ) : apply_filters( 'colorshop_default_catalog_orderby', get_option( 'colorshop_default_catalog_orderby' ) );
		...
		return apply_filters( 'colorshop_get_catalog_ordering_args', $args );
	}
