 ...
 *
 * ColorShop Email Class which is extended by specific email template classes to add emails to ColorShop
 *
 * @class 		CS_Email
 * @version		1.0.0
 ...
 */
abstract class CS_Email extends CS_Settings_API {
	...
	function __construct() {
		...
		// Save settings hook
		add_action( 'colorshop_update_options_email_' . $this->id, array( $this, 'process_admin_options' ) );

		// Default template base if not declared in child constructor
		if ( is_null( $this->template_base ) )
			$this->template_base = $colorshop->plugin_path() . '/templates/';
	...
	function get_subject() {
		return apply_filters( 'colorshop_email_subject_' . $this->id, $this->format_string( $this->subject ), $this->object );
	}
	...
	function get_heading() {
		return apply_filters( 'colorshop_email_heading_' . $this->id, $this->format_string( $this->heading ), $this->object );
	}
	...
	function get_recipient() {
		return apply_filters( 'colorshop_email_recipient_' . $this->id, $this->recipient, $this->object );
	}
	...
	function get_headers() {
		return apply_filters( 'colorshop_email_headers', "Content-Type: " . $this->get_content_type() . "\r\n", $this->id, $this->object );
	}
	...
	function get_attachments() {
		return apply_filters( 'colorshop_email_attachments', '', $this->id, $this->object );
	}
	...
	function is_enabled() {
		...
		return apply_filters( 'colorshop_email_enabled_' . $this->id, $enabled, $this->object );
	}
	...
	function style_inline( $content ) {
		...
		foreach( $nodes_h1 as $node )
			if ( ! $node->hasAttribute( 'style' ) )
				$node->setAttribute( "style", "color: " . get_option( 'colorshop_email_text_color' ) . "; display:block; font-family:Arial; font-size:34px; font-weight:bold; margin-top: 10px; margin-right:0; margin-bottom:10px; margin-left:0; text-align:left; line-height: 150%;" );

		foreach( $nodes_h2 as $node )
			if ( ! $node->hasAttribute( 'style' ) )
				$node->setAttribute( "style", "color: " . get_option( 'colorshop_email_text_color' ) . "; display:block; font-family:Arial; font-size:30px; font-weight:bold; margin-top: 10px; margin-right:0; margin-bottom:10px; margin-left:0; text-align:left; line-height: 150%;" );

		foreach( $nodes_h3 as $node )
			if ( ! $node->hasAttribute( 'style' ) )
				$node->setAttribute( "style", "color: " . get_option( 'colorshop_email_text_color' ) . "; display:block; font-family:Arial; font-size:26px; font-weight:bold; margin-top: 10px; margin-right:0; margin-bottom:10px; margin-left:0; text-align:left; line-height: 150%;" );

		$nodes = $dom->getElementsByTagName('a');

		foreach( $nodes as $node )
			if ( ! $node->hasAttribute( 'style' ) )
				$node->setAttribute( "style", "color: " . get_option( 'colorshop_email_text_color' ) . "; font-weight:normal; text-decoration:underline;" );
	...
	function get_from_name() {
		return wp_specialchars_decode( esc_html( get_option( 'colorshop_email_from_name' ) ) );
	}
	...
	function get_from_address() {
		return sanitize_email( get_option( 'colorshop_email_from_address' ) );
	}
    ...
    public function process_admin_options() {
		...
		if ( ! empty( $_POST['template_html_code'] ) && ! empty( $this->template_html ) ) {
			...
			$saved	= false;
			$file	= get_stylesheet_directory() . '/colorshop/' . $this->template_html;
			...
			if ( ! $saved ) {
    			$redirect = add_query_arg( 'cs_error', urlencode( __( 'Could not write to template file.', 'colorshop' ) ) );
		...
		if ( ! empty( $_POST['template_plain_code'] ) && ! empty( $this->template_plain ) ) {
    		...
    		$saved	= false;
			$file	= get_stylesheet_directory() . '/colorshop/' . $this->template_plain;
			...
			if ( ! $saved ) {
    			$redirect = add_query_arg( 'cs_error', __( 'Could not write to template file.', 'colorshop' ) );
    			wp_redirect( $redirect );
    			exit;
    		}
		}
	...
	function admin_options() {
		...
		if ( ! empty( $this->template_html ) || ! empty( $this->template_plain ) ) {
			...
			if ( ! empty( $_GET['move_template'] ) && ( $template = esc_attr( basename( $_GET['move_template'] ) ) ) ) {
				if ( ! empty( $this->$template ) ) {
					if (  wp_mkdir_p( dirname( get_stylesheet_directory() . '/colorshop/' . $this->$template ) ) && ! file_exists( get_stylesheet_directory() . '/colorshop/' . $this->$template ) ) {
						copy( $this->template_base . $this->$template, get_stylesheet_directory() . '/colorshop/' . $this->$template );
						echo '<div class="updated fade"><p>' . __( 'Template file copied to theme.', 'colorshop' ) . '</p></div>';
					}
				}
			}
			...
			if ( ! empty( $_GET['delete_template'] ) && ( $template = esc_attr( basename( $_GET['delete_template'] ) ) ) ) {
				if ( ! empty( $this->$template ) ) {
					if ( file_exists( get_stylesheet_directory() . '/colorshop/' . $this->$template ) ) {
						unlink( get_stylesheet_directory() . '/colorshop/' . $this->$template );
						echo '<div class="updated fade"><p>' . __( 'Template file deleted from theme.', 'colorshop' ) . '</p></div>';
					}
				}
			}
		...
		?>
		<h3><?php echo ( ! empty( $this->title ) ) ? $this->title : __( 'Settings','colorshop' ) ; ?></h3>
		...
		<?php if ( ! empty( $this->template_html ) || ! empty( $this->template_plain ) ) { ?>
			...
			<?php
				$templates = array(
					'template_html' 	=> __( 'HTML template', 'colorshop' ),
					'template_plain' 	=> __( 'Plain text template', 'colorshop' )
				);
				foreach ( $templates as $template => $title ) :
					if ( empty( $this->$template ) )
						continue;

					$local_file = get_stylesheet_directory() . '/colorshop/' . $this->$template;
					...
					<div class="template <?php echo $template; ?>">
						...
						<?php if ( file_exists( $local_file ) ) : ?>
							...
							<p>
								...
								<?php if ( is_writable( $local_file ) ) : ?>
									<a href="<?php echo remove_query_arg( array( 'move_template', 'saved' ), add_query_arg( 'delete_template', $template ) ); ?>" class="delete_template button"><?php _e( 'Delete template file', 'colorshop' ); ?></a>
								<?php endif; ?>

								<?php printf( __( 'This template has been overridden by your theme and can be found in: <code>%s</code>.', 'colorshop' ), 'yourtheme/colorshop/' . $this->$template ); ?>
							</p>
						...
						<?php elseif ( file_exists( $core_file ) ) : ?>
							...
							<p>
								...
								<?php if ( ( is_dir( get_stylesheet_directory() . '/colorshop/emails/' ) && is_writable( get_stylesheet_directory() . '/colorshop/emails/' ) ) || is_writable( get_stylesheet_directory() ) ) : ?>
									<a href="<?php echo remove_query_arg( array( 'delete_template', 'saved' ), add_query_arg( 'move_template', $template ) ); ?>" class="button"><?php _e( 'Copy file to theme', 'colorshop' ); ?></a>
								<?php endif; ?>

								<?php printf( __( 'To override and edit this email template copy <code>%s</code> to your theme folder: <code>%s</code>.', 'colorshop' ), plugin_basename( $core_file ) , 'yourtheme/colorshop/' . $this->$template ); ?>
							</p>
						...
						<?php else : ?>
							...
							<p><?php _e( 'File was not found.', 'colorshop' ); ?></p>
			...
			$colorshop->add_inline_js("
				...
				var view = '" . __( 'View template', 'colorshop' ) . "';
				var hide = '" . __( 'Hide template', 'colorshop' ) . "';
				...
				jQuery('a.delete_template').click(function(){
					var answer = confirm('" . __( 'Are you sure you want to delete this template file?', 'colorshop' ) . "');
