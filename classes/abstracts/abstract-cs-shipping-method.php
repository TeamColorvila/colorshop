...
/**
 * ColorShop Shipping Method Class
 *
 * Extended by shipping methods to handle shipping calculations etc.
 *
 * @class 		CS_Shipping_Method
 * @version		1.0.0
 * @package		ColorShop/Abstracts
 * @category	Abstract Class
 * @author 		ColorVila
 */
abstract class CS_Shipping_Method extends CS_Settings_API {
	...
	/** @var bool Whether the method has settings or not (In ColorShop > Settings > Shipping) */
	...
	function add_rate( $args = array() ) {
		...
		if ( ! is_array( $taxes ) && $taxes !== false && $total_cost > 0 && get_option( 'colorshop_calc_taxes' ) == 'yes' && $this->tax_status == 'taxable' ) {
		...
		$this->rates[] = new CS_Shipping_Rate( $id, $label, $total_cost, $taxes, $this->id );
	}
    ...
    function is_available( $package ) {
		...
		if ( isset( $colorshop->cart->cart_contents_total ) && isset( $this->min_amount ) && $this->min_amount && $this->min_amount > $colorshop->cart->cart_contents_total )
			return false;
		...
		return apply_filters( 'colorshop_shipping_' . $this->id . '_is_available', true, $package );
    }
	 ...
	 */
	function get_title() {
		return apply_filters( 'colorshop_shipping_method_title', $this->title, $this->id );
	}
	...
	function supports( $feature ) {
		return apply_filters( 'colorshop_shipping_method_supports', in_array( $feature, $this->supports ) ? true : false, $feature, $this );
	}
}