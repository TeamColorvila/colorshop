...
abstract class CS_Settings_API {
	...
	public function admin_options() { ?>
		<h3><?php echo ( ! empty( $this->method_title ) ) ? $this->method_title : __( 'Settings', 'colorshop' ) ; ?></h3>
    ...
    public function process_admin_options() {
    	...
    	if ( count( $this->errors ) > 0 ) {
    		...
    		update_option( $this->plugin_id . $this->id . '_settings', apply_filters( 'colorshop_settings_api_sanitized_fields_' . $this->id, $this->sanitized_fields ) );
    		return true;
    	}
    }
    ...
    public function generate_text_html( $key, $data ) {
		...
		$html .= '<tr valign="top">' . "\n";
			...
			if ( $tip )
				$html .= '<img class="help_tip" data-tip="' . esc_attr( $tip ) . '" src="' . $colorshop->plugin_url() . '/assets/images/help.png" height="16" width="16" />';
    ...
    public function validate_password_field( $key ) {
    	...
    	if ( isset( $_POST[ $this->plugin_id . $this->id . '_' . $key ] ) ) {
    		$text = esc_attr( colorshop_clean( $_POST[ $this->plugin_id . $this->id . '_' . $key ] ) );
    	}
    ...
    public function validate_select_field( $key ) {
    	...
    	if ( isset( $_POST[ $this->plugin_id . $this->id . '_' . $key ] ) ) {
    		$value = esc_attr( colorshop_clean( $_POST[ $this->plugin_id . $this->id . '_' . $key ] ) );
    	}
    ...
    public function validate_multiselect_field( $key ) {
    	...
    	if ( isset( $_POST[ $this->plugin_id . $this->id . '_' . $key ] ) ) {
    		$value = array_map('esc_attr', array_map('colorshop_clean', (array) $_POST[ $this->plugin_id . $this->id . '_' . $key ] ));
    	} else {
	    	$value = '';
    	}
