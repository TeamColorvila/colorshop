 ...
 *
 * The ColorShop product class handles individual product data.
 *
 * @class 		CS_Product
 * @version		1.0.0
...
class CS_Product {
	...
	public function __construct( $product ) {
		...
		if ( ( function_exists( 'get_called_class' ) && get_called_class() == 'CS_Product' ) || ( ! function_exists( 'get_called_class' ) && is_null( $this->product_type ) ) ) {
			_doing_it_wrong( 'CS_Product', __( 'The <code>CS_Product</code> class is now abstract. Use <code>get_product()</code> to instantiate an instance of a product instead of calling this class directly.', 'colorshop' ), '2.0 of ColorShop' );
    ...
    function get_stock_quantity() {
        return $this->managing_stock() ? apply_filters( 'colorshop_stock_amount', $this->stock ) : '';
    }
	...
	function set_stock( $amount = null ) {
		...
		if ( $this->managing_stock() && ! is_null( $amount ) ) {
			...
			$colorshop->clear_product_transients( $this->id ); // Clear transient
	...
	function set_stock_status( $status ) {
		...
		if ( $this->stock_status != $status ) {
			...
			do_action( 'colorshop_product_set_stock_status', $this->id, $status );
		}
	}
	...
	function get_file_download_path( $download_id ) {
		...
		$file_paths = apply_filters( 'colorshop_file_download_paths', $file_paths, $this->id, null, null );
		...
		return apply_filters( 'colorshop_file_download_path', $file_path, $this->id, $download_id );
	}
	...
	function is_sold_individually() {
		...
		return apply_filters( 'colorshop_is_sold_individually', $return, $this );
	}
	 ...
	 */
	function is_taxable() {
		return $this->tax_status == 'taxable' && get_option( 'colorshop_calc_taxes' ) == 'yes' ? true : false;
	}
	 ...
	 */
	function get_title() {
		return apply_filters( 'colorshop_product_title', apply_filters( 'the_title', $this->post->post_title, $this->id ), $this );
	}
	 ...
	 */
	function get_parent() {
		return apply_filters('colorshop_product_parent', $this->post->post_parent, $this);
	}
	...
	function add_to_cart_url() {
		return apply_filters( 'colorshop_add_to_cart_url', add_query_arg( 'add-to-cart', $this->id ) );
	}
	...
	function managing_stock() {
		return ( ! isset( $this->manage_stock ) || $this->manage_stock == 'no' || get_option('colorshop_manage_stock') != 'yes' ) ? false : true;
	}
	...
	function get_availability() {
		...
		if ( $this->managing_stock() ) {
			if ( $this->is_in_stock() ) {
				...
				if ( $this->get_total_stock() > 0 ) {
					...
					$format_option = get_option( 'colorshop_stock_format' );

					switch ( $format_option ) {
						case 'no_amount' :
							$format = __( 'In stock', 'colorshop' );
						break;
						case 'low_amount' :
							$low_amount = get_option( 'colorshop_notify_low_stock_amount' );

							$format = ( $this->get_total_stock() <= $low_amount ) ? __( 'Only %s left in stock', 'colorshop' ) : __( 'In stock', 'colorshop' );
						break;
						default :
							$format = __( '%s in stock', 'colorshop' );
						break;
					}
					...
					if ( $this->backorders_allowed() && $this->backorders_require_notification() )
						$availability .= ' ' . __( '(backorders allowed)', 'colorshop' );

				} else {

					if ( $this->backorders_allowed() ) {
						if ( $this->backorders_require_notification() ) {
							$availability = __( 'Available on backorder', 'colorshop' );
							$class        = 'available-on-backorder';
						} else {
							$availability = __( 'In stock', 'colorshop' );
						}
					} else {
						$availability = __( 'Out of stock', 'colorshop' );
						$class        = 'out-of-stock';
					}
		...
		return apply_filters( 'colorshop_get_availability', array( 'availability' => $availability, 'class' => $class ), $this );
	}
	...
	function is_visible() {
		...
		if ( get_option( 'colorshop_hide_out_of_stock_items' ) == 'yes' && ! $this->is_in_stock() ) $visible = false;
		...
		return apply_filters( 'colorshop_product_is_visible', $visible, $this->id );
	}
	 ...
	 */
	function get_price() {
		return apply_filters( 'colorshop_get_price', $this->price, $this );
	}
	...
	function is_purchasable() {
		...
		return apply_filters( 'colorshop_is_purchasable', $purchasable, $this );
	}
	...
	function get_price_including_tax( $qty = 1 ) {
		...
		if ( $this->is_taxable() ) {
			...
			if ( get_option('colorshop_prices_include_tax') == 'no' ) {
		...
		return apply_filters( 'colorshop_get_price_including_tax', $price, $qty, $this );
	}
	...
	function get_price_excluding_tax( $qty = 1 ) {
		...
		if ( $this->is_taxable() && get_option('colorshop_prices_include_tax') == 'yes' ) {
		...
		return apply_filters( 'colorshop_get_price_excluding_tax', $price, $qty, $this );
	}
	...
	function get_tax_class() {
		return apply_filters( 'colorshop_product_tax_class', $this->tax_class, $this );
	}
	...
	function get_price_html( $price = '' ) {
		...
		if ( $this->price > 0 ) {
			...
			if ( $this->is_on_sale() && isset( $this->regular_price ) ) {
				...
				$price = apply_filters( 'colorshop_sale_price_html', $price, $this );

			} else {

				$price .= colorshop_price( $this->get_price() );

				$price = apply_filters( 'colorshop_price_html', $price, $this );

			}
		} elseif ( $this->price === '' ) {

			$price = apply_filters( 'colorshop_empty_price_html', '', $this );

		} elseif ( $this->price == 0 ) {

			if ( $this->is_on_sale() && isset( $this->regular_price ) ) {

				$price .= $this->get_price_html_from_to( $this->regular_price, __( 'Free!', 'colorshop' ) );

				$price = apply_filters( 'colorshop_free_sale_price_html', $price, $this );

			} else {

				$price = __( 'Free!', 'colorshop' );

				$price = apply_filters( 'colorshop_free_price_html', $price, $this );

			}
		}

		return apply_filters( 'colorshop_get_price_html', $price, $this );
	}
	...
	function get_price_html_from_text() {
		return '<span class="from">' . _x('From:', 'min_price', 'colorshop') . ' </span>';
	}
	...
	function get_price_html_from_to( $from, $to ) {
		return '<del>' . ( ( is_numeric( $from ) ) ? colorshop_price( $from ) : $from ) . '</del> <ins>' . ( ( is_numeric( $to ) ) ? colorshop_price( $to ) : $to ) . '</ins>';
	}
	...
	function get_average_rating() {
		if ( false === ( $average_rating = get_transient( 'cs_average_rating_' . $this->id ) ) ) {
			...
			set_transient( 'cs_average_rating_' . $this->id, $average_rating );
		}
	...
	function get_rating_count() {
		if ( false === ( $count = get_transient( 'cs_rating_count_' . $this->id ) ) ) {
			...
			set_transient( 'cs_rating_count_' . $this->id, $count );
		}
	...
	function get_rating_html( $location = '' ) {
		...
		if ( $average_rating > 0 ) {
			...
			$rating_html  = '<div class="star-rating" title="' . sprintf( __( 'Rated %s out of 5', 'colorshop' ), $average_rating ) . '">';

			$rating_html .= '<span style="width:' . ( ( $average_rating / 5 ) * 100 ) . '%"><strong class="rating">' . $average_rating . '</strong> ' . __( 'out of 5', 'colorshop' ) . '</span>';
	...
	function get_related( $limit = 5 ) {
		...
		// Get the posts
		$related_posts = get_posts( apply_filters('colorshop_product_related_posts', array(
			'orderby'        => 'rand',
	...
	function get_attribute( $attr ) {
		...
		if ( isset( $attributes[ $attr ] ) || isset( $attributes[ 'pa_' . $attr ] ) ) {
			...
			if ( $attribute['is_taxonomy'] ) {
				...
				return implode( ', ', colorshop_get_product_terms( $this->id, $attribute['name'], 'names' ) );
	...
	function enable_dimensions_display() {
		return get_option( 'colorshop_enable_dimension_product_attributes' ) == 'yes' ? true : false;
	}
	...
	function get_dimensions() {
		if ( ! $this->dimensions ) {
			...
			if ( ! empty( $this->dimensions ) )
				$this->dimensions .= ' ' . get_option( 'colorshop_dimension_unit' );
	...
	function list_attributes() {
		colorshop_get_template( 'single-product/product-attributes.php', array(
			'product'    => $this
		) );
	}
    ...
    function get_image( $size = 'shop_thumbnail', $attr = array() ) {
		...
		if ( has_post_thumbnail( $this->id ) ) {
			...
			$image = colorshop_placeholder_img( $size );
		}
