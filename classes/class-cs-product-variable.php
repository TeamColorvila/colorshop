 ...
 *
 * @class 		CS_Product_Variable
 * @version		1.0.0
 * @package		ColorShop/Classes/Products
 * @category	Class
 * @author 		ColorVila
 */
class CS_Product_Variable extends CS_Product {
    ...
    public function get_total_stock() {
        ...
        if ( empty( $this->total_stock ) ) {
        	...
        	$transient_name = 'cs_product_total_stock_' . $this->id;
		...
		return apply_filters( 'colorshop_stock_amount', $this->total_stock );
    }
	...
	function set_stock( $amount = null ) {
		...
		if ( $this->managing_stock() && ! is_null( $amount ) ) {
			...
			return apply_filters( 'colorshop_stock_amount', $this->stock );
		}
	}
	...
	public function get_children() {
		...
		if ( ! is_array( $this->children ) ) {
			...
			$transient_name = 'cs_product_children_ids_' . $this->id;
	 ...
	 * @return object CS_Product or CS_Product_variation
	 */
	...
	public function get_price_html( $price = '' ) {
		...
		if ( $this->price > 0 ) {
			if ( $this->is_on_sale() && isset( $this->min_variation_price ) && $this->min_variation_regular_price !== $this->get_price() ) {
				...
				$price = apply_filters( 'colorshop_variable_sale_price_html', $price, $this );
				...
				$price = apply_filters('colorshop_variable_price_html', $price, $this);

			}
		} elseif ( $this->price === '' ) {

			$price = apply_filters('colorshop_variable_empty_price_html', '', $this);
			...
			if ( $this->is_on_sale() && isset( $this->min_variation_regular_price ) && $this->min_variation_regular_price !== $this->get_price() ) {
				...
				$price .= $this->get_price_html_from_to( $this->min_variation_regular_price, __( 'Free!', 'colorshop' ) );

				$price = apply_filters( 'colorshop_variable_free_sale_price_html', $price, $this );
				...
				$price = apply_filters( 'colorshop_variable_free_price_html', $price, $this );
    ...
    public function get_variation_default_attributes() {
	    ...
	    return apply_filters( 'colorshop_product_default_attributes', (array) maybe_unserialize( $default ), $this );
    }
    ...
    public function get_available_variations() {
		...
		foreach ( $this->get_children() as $child_id ) {
			...
			if ( ! empty( $variation->variation_id ) ) {
				...
				$availability_html 		= empty( $availability['availability'] ) ? '' : apply_filters( 'colorshop_stock_html', '<p class="stock ' . esc_attr( $availability['class'] ) . '">'. wp_kses_post( $availability['availability'] ).'</p>', wp_kses_post( $availability['availability'] ) );
				...
				$available_variations[] = apply_filters( 'colorshop_available_variation', array(
					...
					'weight'				=> $variation->get_weight() . ' ' . esc_attr( get_option('colorshop_weight_unit' ) ),
	...
	public function variable_product_sync() {
		...
		if ( $children ) {
			...
			$colorshop->clear_product_transients( $this->id );
		}
	}
}