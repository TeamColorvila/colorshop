 ...
 * ColorShop cart
 *
 * The ColorShop cart class stores cart data and active coupons as well as handling customer sessions and some cart related urls.
...
class CS_Cart {
	...
	public function __construct() {
		$this->tax = new CS_Tax();
		$this->prices_include_tax = ( get_option( 'colorshop_prices_include_tax' ) == 'yes' ) ? true : false;
		$this->tax_display_cart = get_option( 'colorshop_tax_display_cart' );
		$this->dp = (int) get_option( 'colorshop_price_num_decimals' );
    ...
    public function init() {
		...
		add_action('colorshop_check_cart_items', array( $this, 'check_cart_items' ), 1 );
		add_action('colorshop_check_cart_items', array( $this, 'check_cart_coupons' ), 1 );
		add_action('colorshop_after_checkout_validation', array( $this, 'check_customer_coupons' ), 1 );
    }
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function get_cart_from_session() {
			...
			// Load the coupons
			$this->applied_coupons         = ( empty( $colorshop->session->coupon_codes ) ) ? array() : array_filter( (array) $colorshop->session->coupon_codes );
			$this->coupon_discount_amounts = ( empty( $colorshop->session->coupon_amounts ) ) ? array() : array_filter( (array) $colorshop->session->coupon_amounts );

			// Load the cart
			if ( isset( $colorshop->session->cart ) && is_array( $colorshop->session->cart ) ) {
				$cart = $colorshop->session->cart;
				...
				foreach ( $cart as $key => $values ) {
					...
					if ( ! empty( $_product ) && $_product->exists() && $values['quantity'] > 0 ) {
						...
						$this->cart_contents[ $key ] = apply_filters( 'colorshop_get_cart_item_from_session', array(
				...
				do_action( 'colorshop_cart_loaded_from_session', $this );
			...
			if ( sizeof( $this->cart_contents ) > 0 )
				$colorshop->cart_has_contents_cookie( true );
			else
				$colorshop->cart_has_contents_cookie( false );

			// Load totals
			$this->cart_contents_total 	= isset( $colorshop->session->cart_contents_total ) ? $colorshop->session->cart_contents_total : 0;
			$this->cart_contents_weight = isset( $colorshop->session->cart_contents_weight ) ? $colorshop->session->cart_contents_weight : 0;
			$this->cart_contents_count 	= isset( $colorshop->session->cart_contents_count ) ? $colorshop->session->cart_contents_count : 0;
			$this->cart_contents_tax 	= isset( $colorshop->session->cart_contents_tax ) ? $colorshop->session->cart_contents_tax : 0;
			$this->total 				= isset( $colorshop->session->total ) ? $colorshop->session->total : 0;
			$this->subtotal 			= isset( $colorshop->session->subtotal ) ? $colorshop->session->subtotal : 0;
			$this->subtotal_ex_tax 		= isset( $colorshop->session->subtotal_ex_tax ) ? $colorshop->session->subtotal_ex_tax : 0;
			$this->tax_total 			= isset( $colorshop->session->tax_total ) ? $colorshop->session->tax_total : 0;
			$this->taxes 				= isset( $colorshop->session->taxes ) ? $colorshop->session->taxes : array();
			$this->shipping_taxes		= isset( $colorshop->session->shipping_taxes ) ? $colorshop->session->shipping_taxes : array();
			$this->discount_cart 		= isset( $colorshop->session->discount_cart ) ? $colorshop->session->discount_cart : 0;
			$this->discount_total 		= isset( $colorshop->session->discount_total ) ? $colorshop->session->discount_total : 0;
			$this->shipping_total 		= isset( $colorshop->session->shipping_total ) ? $colorshop->session->shipping_total : 0;
			$this->shipping_tax_total 	= isset( $colorshop->session->shipping_tax_total ) ? $colorshop->session->shipping_tax_total : 0;
			$this->shipping_label		= isset( $colorshop->session->shipping_label ) ? $colorshop->session->shipping_label : '';
		...
		public function set_session() {
			...
			$colorshop->session->cart           = $cart_session;
			$colorshop->session->coupon_codes   = $this->applied_coupons;
			$colorshop->session->coupon_amounts = $this->coupon_discount_amounts;

			// Store totals to avoid re-calc on page load
			$colorshop->session->cart_contents_total  = $this->cart_contents_total;
			$colorshop->session->cart_contents_weight = $this->cart_contents_weight;
			$colorshop->session->cart_contents_count  = $this->cart_contents_count;
			$colorshop->session->cart_contents_tax    = $this->cart_contents_tax;
			$colorshop->session->total                = $this->total;
			$colorshop->session->subtotal             = $this->subtotal;
			$colorshop->session->subtotal_ex_tax      = $this->subtotal_ex_tax;
			$colorshop->session->tax_total            = $this->tax_total;
			$colorshop->session->shipping_taxes       = $this->shipping_taxes;
			$colorshop->session->taxes                = $this->taxes;
			$colorshop->session->discount_cart        = $this->discount_cart;
			$colorshop->session->discount_total       = $this->discount_total;
			$colorshop->session->shipping_total       = $this->shipping_total;
			$colorshop->session->shipping_tax_total   = $this->shipping_tax_total;
			$colorshop->session->shipping_label       = $this->shipping_label;

			if ( get_current_user_id() )
				$this->persistent_cart_update();

			do_action( 'colorshop_cart_updated' );
		}
		...
		public function empty_cart( $clear_persistent_cart = true ) {
			...
			unset( $colorshop->session->order_awaiting_payment, $colorshop->session->coupon_codes, $colorshop->session->coupon_amounts, $colorshop->session->cart );

			if ( $clear_persistent_cart && get_current_user_id() )
				$this->persistent_cart_destroy();

			do_action( 'colorshop_cart_emptied' );
		}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function persistent_cart_update() {
			...
			update_user_meta( get_current_user_id(), '_colorshop_persistent_cart', array(
				'cart' => $colorshop->session->cart,
			) );
		}
		...
		public function persistent_cart_destroy() {
			delete_user_meta( get_current_user_id(), '_colorshop_persistent_cart' );
		}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function coupons_enabled() {
			$coupons_enabled = get_option( 'colorshop_enable_coupons' ) == 'no' ? false : true;

			return apply_filters( 'colorshop_coupons_enabled', $coupons_enabled );
		}
		...
		public function get_cart_contents_count() {
			return apply_filters( 'colorshop_cart_contents_count', $this->cart_contents_count );
		}
		...
		public function check_cart_items() {
			...
			if (is_wp_error($result))
				$colorshop->add_error( $result->get_error_message() );
		}
			...
			if ( ! empty( $this->applied_coupons ) ) {
				foreach ( $this->applied_coupons as $key => $code ) {
					...
					if ( is_wp_error( $coupon->is_valid() ) ) {
						...
						$coupon->add_coupon_message( CS_Coupon::E_CS_COUPON_INVALID_REMOVED );

						// Remove the coupon
						unset( $this->applied_coupons[ $key ] );

						$colorshop->session->coupon_codes   = $this->applied_coupons;
						$colorshop->session->refresh_totals     = true;
					}
				}
			}
		}
				...
				foreach ( $this->applied_coupons as $key => $code ) {
					...
					if ( ! is_wp_error( $coupon->is_valid() ) && is_array( $coupon->customer_email ) && sizeof( $coupon->customer_email ) > 0 ) {
						...
						if ( 0 == sizeof( array_intersect( $check_emails, $coupon->customer_email ) ) ) {
							$coupon->add_coupon_message( CS_Coupon::E_CS_COUPON_NOT_YOURS_REMOVED );
			...
			foreach ( $this->get_cart() as $cart_item_key => $values ) {
				...
				if ( $_product->managing_stock() ) {
					...
					if ( ! $_product->is_in_stock() || ! $_product->has_enough_stock( $values['quantity'] ) ) {
						$error->add( 'out-of-stock', sprintf(__( 'Sorry, we do not have enough "%s" in stock to fulfill your order (%s in stock). Please edit your cart and try again. We apologise for any inconvenience caused.', 'colorshop' ), $_product->get_title(), $_product->stock ) );
						return $error;
					}
					...
					if ( $values['variation_id'] && $_product->variation_has_stock && isset( $product_qty_in_cart[ $values['variation_id'] ] ) ) {
						...
						if ( ! $_product->has_enough_stock( $product_qty_in_cart[ $values['variation_id'] ] ) ) {
							$error->add( 'out-of-stock', sprintf(__( 'Sorry, we do not have enough "%s" in stock to fulfil your order (%s in stock). Please edit your cart and try again. We apologise for any inconvenience caused.', 'colorshop' ), $_product->get_title(), $_product->stock ) );
							return $error;
						}
					 ...
					 */
					if ( get_option( 'colorshop_hold_stock_minutes' ) > 0 && ! $_product->backorders_allowed() ) {

						$order_id = isset( $colorshop->session->order_awaiting_payment ) ? absint( $colorshop->session->order_awaiting_payment ) : 0;
						...
						$held_stock = $wpdb->get_var( $wpdb->prepare( "
							...
							LEFT JOIN {$wpdb->prefix}colorshop_order_items as order_items ON posts.ID = order_items.order_id
							...
							LEFT JOIN {$wpdb->prefix}colorshop_order_itemmeta as order_item_meta2 ON order_items.order_item_id = order_item_meta2.order_item_id
						...
						if ( $_product->stock < ( $held_stock + $in_cart ) ) {
							$error->add( 'out-of-stock', sprintf(__( 'Sorry, we do not have enough "%s" in stock to fulfil your order right now. Please try again in %d minutes or edit your cart and try again. We apologise for any inconvenience caused.', 'colorshop' ), $_product->get_title(), get_option( 'colorshop_hold_stock_minutes' ) ) );
							return $error;
						}
					}
				 ...
				 */
					...
					if ( ! $_product->is_in_stock() ) {
						$error->add( 'out-of-stock', sprintf(__( 'Sorry, "%s" is not in stock. Please edit your cart and try again. We apologise for any inconvenience caused.', 'colorshop' ), $_product->get_title() ) );
						return $error;
					}
				}
			}
		...
		public function get_item_data( $cart_item, $flat = false ) {
			...
			if ( ! empty( $cart_item['data']->variation_id ) && is_array( $cart_item['variation'] ) ) {
				...
				foreach ( $cart_item['variation'] as $name => $value ) {
		            ...
		            // If this is a custom option slug, get the options name
		            	...
		            	$value = apply_filters( 'colorshop_variation_option_name', $value );
					}
			...
			$other_data = apply_filters( 'colorshop_get_item_data', array(), $cart_item );
		...
		public function get_cart_url() {
			$cart_page_id = colorshop_get_page_id('cart');
			if ( $cart_page_id ) return apply_filters( 'colorshop_get_cart_url', get_permalink( $cart_page_id ) );
		}
		...
		public function get_checkout_url() {
			$checkout_page_id = colorshop_get_page_id('checkout');
			if ( $checkout_page_id ) {
				if ( is_ssl() )
					return str_replace( 'http:', 'https:', get_permalink($checkout_page_id) );
				else
					return apply_filters( 'colorshop_get_checkout_url', get_permalink($checkout_page_id) );
			}
		}
		...
		public function get_remove_url( $cart_item_key ) {
			...
			$cart_page_id = colorshop_get_page_id('cart');
			if ($cart_page_id)
				return apply_filters( 'colorshop_get_remove_url', $colorshop->nonce_url( 'cart', add_query_arg( 'remove_item', $cart_item_key, get_permalink($cart_page_id) ) ) );
		}
		...
		public function get_formatted_taxes() {
			...
			foreach ( $taxes as $key => $tax )
				if ( is_numeric( $tax ) )
					$taxes[ $key ] = colorshop_price( $tax );

			return apply_filters( 'colorshop_cart_formatted_taxes', $taxes, $this );
		}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function add_to_cart( $product_id, $quantity = 1, $variation_id = '', $variation = '', $cart_item_data = array() ) {
			...
			$cart_item_data = (array) apply_filters( 'colorshop_add_cart_item_data', $cart_item_data, $product_id, $variation_id );
			...
			if ( ! $product_data->is_purchasable() ) {
				$colorshop->add_error( sprintf( __( 'Sorry, &quot;%s&quot; cannot be purchased.', 'colorshop' ), $product_data->get_title() ) );
				return false;
			}
			...
			if ( ! $product_data->is_in_stock() ) {
				...
				$colorshop->add_error( sprintf( __( 'You cannot add &quot;%s&quot; to the cart because the product is out of stock.', 'colorshop' ), $product_data->get_title() ) );

				return false;
			} elseif ( ! $product_data->has_enough_stock( $quantity ) ) {

				$colorshop->add_error( sprintf(__( 'You cannot add that amount of &quot;%s&quot; to the cart because there is not enough stock (%s remaining).', 'colorshop' ), $product_data->get_title(), $product_data->get_stock_quantity() ));
			...
			if ( $product_data->is_sold_individually() ) {
				...
				if ( $in_cart_quantity > 0 ) {
					$colorshop->add_error( sprintf('<a href="%s" class="button">%s</a> %s', get_permalink(colorshop_get_page_id('cart')), __( 'View Cart &rarr;', 'colorshop' ), __( 'You already have this item in your cart.', 'colorshop' ) ) );
					return false;
				}
			}
			...
			if ( $product_data->managing_stock() ) {
				...
				if ( $variation_id && $product_data->variation_has_stock ) {
					...
					if ( isset( $product_qty_in_cart[ $variation_id ] ) && ! $product_data->has_enough_stock( $product_qty_in_cart[ $variation_id ] + $quantity ) ) {
						$colorshop->add_error( sprintf(__( '<a href="%s" class="button">%s</a> You cannot add that amount to the cart &mdash; we have %s in stock and you already have %s in your cart.', 'colorshop' ), get_permalink(colorshop_get_page_id('cart')), __( 'View Cart &rarr;', 'colorshop' ), $product_data->get_stock_quantity(), $product_qty_in_cart[ $variation_id ] ));
						return false;
					}
				...
				// Products
					...
					if ( isset( $product_qty_in_cart[ $product_id ] ) && ! $product_data->has_enough_stock( $product_qty_in_cart[ $product_id ] + $quantity ) ) {
						$colorshop->add_error( sprintf(__( '<a href="%s" class="button">%s</a> You cannot add that amount to the cart &mdash; we have %s in stock and you already have %s in your cart.', 'colorshop' ), get_permalink(colorshop_get_page_id('cart')), __( 'View Cart &rarr;', 'colorshop' ), $product_data->get_stock_quantity(), $product_qty_in_cart[ $product_id ] ));
						return false;
					}
			...
			if ( $cart_item_key ) {
				...
				$this->cart_contents[$cart_item_key] = apply_filters( 'colorshop_add_cart_item', array_merge( $cart_item_data, array(
			...
			do_action( 'colorshop_add_to_cart', $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data );
		...
		public function set_quantity( $cart_item_key, $quantity = 1 ) {
			...
			if ( $quantity == 0 || $quantity < 0 ) {
				do_action( 'colorshop_before_cart_item_quantity_zero', $cart_item_key );
				unset( $this->cart_contents[$cart_item_key] );
			} else {
				$this->cart_contents[$cart_item_key]['quantity'] = $quantity;
				do_action( 'colorshop_after_cart_item_quantity_update', $cart_item_key, $quantity );
			}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		private function reset() {
			...
			unset( $colorshop->session->cart_contents_total, $colorshop->session->cart_contents_weight, $colorshop->session->cart_contents_count, $colorshop->session->cart_contents_tax, $colorshop->session->total, $colorshop->session->subtotal, $colorshop->session->subtotal_ex_tax, $colorshop->session->tax_total, $colorshop->session->taxes, $colorshop->session->shipping_taxes, $colorshop->session->discount_cart, $colorshop->session->discount_total, $colorshop->session->shipping_total, $colorshop->session->shipping_tax_total, $colorshop->session->shipping_label );
		}
		...
		public function get_discounted_price( $values, $price, $add_totals = false ) {
					...
					if ( $coupon->apply_before_tax() && $coupon->is_valid() ) {
						...
						switch ( $coupon->type ) {
							...
							case "percent_product" :
								...
								// Apply filter
								$this_item_is_discounted = apply_filters( 'colorshop_item_is_discounted', $this_item_is_discounted, $values, $before_tax = true, $coupon );
			...
			return apply_filters( 'colorshop_get_discounted_price', $price, $values, $this );
		}
		...
		public function apply_product_discounts_after_tax( $values, $price ) {
			...
			if ( ! empty( $this->applied_coupons) ) {
				foreach ( $this->applied_coupons as $code ) {
					...
					do_action( 'colorshop_product_discount_after_tax_' . $coupon->type, $coupon, $values, $price );
					...
					if ( ! $coupon->apply_before_tax() ) {
						...
						// Apply filter
						$this_item_is_discounted = apply_filters( 'colorshop_item_is_discounted', $this_item_is_discounted, $values, $before_tax = false, $coupon );
		...
		public function apply_cart_discounts_after_tax() {
			...
			if ( $this->applied_coupons ) {
				foreach ( $this->applied_coupons as $code ) {
					...
					do_action( 'colorshop_cart_discount_after_tax_' . $coupon->type, $coupon );
		...
		public function calculate_totals() {
			...
			do_action( 'colorshop_before_calculate_totals', $this );
				...
				foreach ( $this->cart_contents as $cart_item_key => $values ) {
					...
					if ( $this->prices_include_tax ) {
						...
						if ( $_product->is_taxable() ) {
							...
							$tax_amount = get_option('colorshop_tax_round_at_subtotal') == 'no' ? $this->tax->get_tax_total( $taxes ) : array_sum( $taxes );
				...
				if ( sizeof($this->cart_contents) > 0 ) {
					foreach ($this->cart_contents as $cart_item_key => $values ) {
						...
						if ( $_product->is_taxable() ) {
							 ...
							 */
							if ( ( $colorshop->customer->is_customer_outside_base() && ( defined('COLORSHOP_CHECKOUT') || $colorshop->customer->has_calculated_shipping() ) ) || ( $_product->get_tax_class() !== $_product->tax_class ) ) {
								...
								// Line subtotal + tax
								$line_subtotal_tax 		= get_option('colorshop_tax_round_at_subtotal') == 'no' ? $this->tax->round( $tax_amount ) : $tax_amount;
						...
						// Line prices
						$line_tax 		= get_option('colorshop_tax_round_at_subtotal') == 'no' ? $this->tax->round( $discounted_tax_amount ) : $discounted_tax_amount;
			...
			if ( $colorshop->customer->is_vat_exempt() ) {
			...
			if ( is_checkout() || is_cart() || defined('COLORSHOP_CHECKOUT') || defined('COLORSHOP_CART') ) {
				...
				do_action( 'colorshop_calculate_totals', $this );
				 ...
				 */
				$this->total = apply_filters( 'colorshop_calculated_total', number_format( $this->cart_contents_total + $this->tax_total + $this->shipping_tax_total + $this->shipping_total - $this->discount_total + $this->fee_total, $this->dp, '.', '' ), $this );
		...
		public function needs_payment() {
			...
			return apply_filters( 'colorshop_cart_needs_payment', $needs_payment, $this );
		}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function calculate_shipping() {
			...
			if ( $this->needs_shipping() && $this->show_shipping() ) {
				$colorshop->shipping->calculate_shipping( $this->get_shipping_packages() );
			} else {
				$colorshop->shipping->reset_shipping();
			}

			// Get totals for the chosen shipping method
			$this->shipping_total 		= $colorshop->shipping->shipping_total;	// Shipping Total
			$this->shipping_label 		= $colorshop->shipping->shipping_label;	// Shipping Label
			$this->shipping_taxes		= $colorshop->shipping->shipping_taxes;	// Shipping Taxes
		...
		public function get_shipping_packages() {
			...
			$packages[0]['destination']['country']   = $colorshop->customer->get_shipping_country();
			$packages[0]['destination']['state']     = $colorshop->customer->get_shipping_state();
			$packages[0]['destination']['postcode']  = $colorshop->customer->get_shipping_postcode();
			$packages[0]['destination']['city']      = $colorshop->customer->get_shipping_city();
			$packages[0]['destination']['address']   = $colorshop->customer->get_shipping_address();
			$packages[0]['destination']['address_2'] = $colorshop->customer->get_shipping_address_2();

			foreach ( $this->get_cart() as $item )
				if ( $item['data']->needs_shipping() )
					$packages[0]['contents_cost'] += $item['line_total'];

			return apply_filters( 'colorshop_cart_shipping_packages', $packages );
		}
		...
		public function needs_shipping() {
			if ( get_option('colorshop_calc_shipping')=='no' ) return false;
			...
			return apply_filters( 'colorshop_cart_needs_shipping', $needs_shipping );
		}
		...
		public function show_shipping() {
			...
			if ( get_option( 'colorshop_shipping_cost_requires_address' ) == 'yes' ) {
				if ( ! $colorshop->customer->has_calculated_shipping() ) {
					if ( ! $colorshop->customer->get_shipping_country() || ( ! $colorshop->customer->get_shipping_state() && ! $colorshop->customer->get_shipping_postcode() ) ) return false;
				}
			}

			$show_shipping = true;

			return apply_filters( 'colorshop_cart_ready_to_calc_shipping', $show_shipping );
		...
		public function ship_to_billing_address_only() {
			if ( get_option('colorshop_ship_to_billing_address_only') == 'yes' ) return true; else return false;
		}
			...
			if ( isset( $this->shipping_label ) ) {
				if ( $this->shipping_total > 0 ) {
					...
					if ( $this->tax_display_cart == 'excl' ) {
						...
						$return = colorshop_price( $this->shipping_total );
						...
						$return = colorshop_price( $this->shipping_total + $this->shipping_tax_total );

						if ( $this->shipping_tax_total > 0 && ! $this->prices_include_tax ) {
							$return .= ' <small>' . $colorshop->countries->inc_tax_or_vat() . '</small>';
						}
		...
		public function get_cart_shipping_title() {
			if ( isset( $this->shipping_label ) ) {
				return __( 'via', 'colorshop' ) . ' ' . $this->shipping_label;
			}
			return false;
		}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function add_discount( $coupon_code ) {
			...
			$the_coupon = new CS_Coupon( $coupon_code );

			if ( $the_coupon->id ) {

				// Check it can be used with cart
				if ( ! $the_coupon->is_valid() ) {
					$colorshop->add_error( $the_coupon->get_error_message() );
					return false;
				}

				// Check if applied
				if ( $colorshop->cart->has_discount( $coupon_code ) ) {
					$the_coupon->add_coupon_message( CS_Coupon::E_CS_COUPON_ALREADY_APPLIED );
					return false;
				}

				// If its individual use then remove other coupons
				if ( $the_coupon->individual_use == 'yes' ) {
					$this->applied_coupons = apply_filters( 'colorshop_apply_individual_use_coupon', array(), $the_coupon, $this->applied_coupons );
				}

				if ( $this->applied_coupons ) {
					foreach ( $this->applied_coupons as $code ) {

						$existing_coupon = new CS_Coupon( $code );

						if ( $existing_coupon->individual_use == 'yes' && false === apply_filters( 'colorshop_apply_with_individual_use_coupon', false, $the_coupon, $existing_coupon, $this->applied_coupons ) ) {

							// Reject new coupon
							$existing_coupon->add_coupon_message( CS_Coupon::E_CS_COUPON_ALREADY_APPLIED_INDIV_USE_ONLY );
				...
				if ( $the_coupon->enable_free_shipping() ) {
					$colorshop->session->chosen_shipping_method = 'free_shipping';
				}

				$this->calculate_totals();

				$the_coupon->add_coupon_message( CS_Coupon::CS_COUPON_SUCCESS );

				do_action( 'colorshop_applied_coupon', $coupon_code );

				return true;

			} else {
				$the_coupon->add_coupon_message( CS_Coupon::E_CS_COUPON_NOT_EXIST );
				return false;
			}
			return false;
		}
		...
		public function remove_coupons( $type = 0 ) {
			...
			if ( 1 == $type ) {
				...
				unset( $colorshop->session->coupon_codes, $colorshop->session->coupon_amounts );
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function get_total() {
			return apply_filters( 'colorshop_cart_total', colorshop_price( $this->total ) );
		}
		...
		public function get_total_ex_tax() {
			...
			if ( $total < 0 ) $total = 0;
			return apply_filters( 'colorshop_cart_total_ex_tax', colorshop_price( $total ) );
		}
		...
		public function get_cart_total() {
			if ( ! $this->prices_include_tax ) {
				$cart_contents_total = colorshop_price( $this->cart_contents_total );
			} else {
				$cart_contents_total = colorshop_price( $this->cart_contents_total + $this->tax_total );
			}

			return apply_filters( 'colorshop_cart_contents_total', $cart_contents_total );
		}
		...
		public function get_cart_subtotal( $compound = false ) {
			...
			if ( $compound ) {
				...
				$cart_subtotal = colorshop_price( $this->cart_contents_total + $this->shipping_total + $this->get_taxes_total( false ) );
			...
			// Otherwise we show cart items totals only (before discount)
				...
				if ( $this->tax_display_cart == 'excl' ) {
					...
					$cart_subtotal = colorshop_price( $this->subtotal_ex_tax );

					if ( $this->tax_total > 0 && $this->prices_include_tax ) {
						$cart_subtotal .= ' <small>' . $colorshop->countries->ex_tax_or_vat() . '</small>';
					}

				} else {

					$cart_subtotal = colorshop_price( $this->subtotal );

					if ( $this->tax_total > 0 && !$this->prices_include_tax ) {
						$cart_subtotal .= ' <small>' . $colorshop->countries->inc_tax_or_vat() . '</small>';
					}

				}
			}

			return apply_filters( 'colorshop_cart_subtotal', $cart_subtotal, $compound, $this );
		}
		...
		public function get_product_subtotal( $_product, $quantity ) {
			...
			if ( $taxable ) {
				...
				if ( $this->tax_display_cart == 'excl' ) {
					...
					$product_subtotal = colorshop_price( $row_price );

					if ( $this->prices_include_tax && $this->tax_total > 0 )
						$product_subtotal .= ' <small class="tax_label">' . $colorshop->countries->ex_tax_or_vat() . '</small>';
					...
					if ( ! $this->prices_include_tax && $this->tax_total > 0 )
						$product_subtotal .= ' <small class="tax_label">' . $colorshop->countries->inc_tax_or_vat() . '</small>';
			...
			return apply_filters( 'colorshop_cart_product_subtotal', $product_subtotal, $_product, $quantity, $this );
		}
		...
		public function get_cart_tax() {
			...
			if ( $cart_total_tax > 0 ) $return = colorshop_price( $cart_total_tax );
			return apply_filters( 'colorshop_get_cart_tax', $return );
		}
		...
		public function get_discounts_before_tax() {
			if ( $this->discount_cart ) {
				$discounts_before_tax = colorshop_price( $this->discount_cart );
			} else {
				$discounts_before_tax = false;
			}
			return apply_filters( 'colorshop_cart_discounts_before_tax', $discounts_before_tax, $this );
		}
		...
		public function get_discounts_after_tax() {
			if ( $this->discount_total ) {
				$discounts_after_tax = colorshop_price( $this->discount_total );
			} else {
				$discounts_after_tax = false;
			}
			return apply_filters( 'colorshop_cart_discounts_after_tax', $discounts_after_tax, $this );
		}
		...
		public function get_total_discount() {
			if ( $this->discount_total || $this->discount_cart ) {
				$total_discount = colorshop_price( $this->discount_total + $this->discount_cart );
			} else {
				$total_discount = false;
			}
			return apply_filters( 'colorshop_cart_total_discount', $total_discount, $this );
		}
}