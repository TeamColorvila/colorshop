 ...
 *
 * The ColorShop product variation class handles product variation data.
 *
 * @class 		CS_Product_Variation
 * @version		1.0.0
 * @package		ColorShop/Classes
 * @category	Class
 * @author 		ColorVila
 */
class CS_Product_Variation extends CS_Product {
	...
	public function is_visible() {
		...
		if ( get_option('colorshop_hide_out_of_stock_items') == 'yes' && ! $this->is_in_stock() )
			$visible = false;
	...
	public function get_price_html() {
		...
		if ( $this->price !== '' ) {
			if ( $this->price == $this->sale_price && $this->sale_price < $this->regular_price ) {
				...
				$price = '<del>' . colorshop_price( $this->regular_price ) . '</del> <ins>' . colorshop_price( $this->sale_price ) . '</ins>';
				$price = apply_filters( 'colorshop_variation_sale_price_html', $price, $this );

			} elseif ( $this->price > 0 ) {

				$price = colorshop_price( $this->price );
				$price = apply_filters( 'colorshop_variation_price_html', $price, $this );

			} else {

				$price = __( 'Free!', 'colorshop' );
				$price = apply_filters( 'colorshop_variation_free_price_html', $price, $this );

			}
		} else {
			$price = apply_filters( 'colorshop_variation_empty_price_html', '', $this );
		}
	...
	public function get_file_download_path( $download_id ) {
		...
		$file_path = '';
		$file_paths = apply_filters( 'colorshop_file_download_paths', get_post_meta( $this->variation_id, '_file_paths', true ), $this->variation_id, null, null );
		...
		return apply_filters( 'colorshop_file_download_path', $file_path, $this->variation_id, $download_id );
	}
}