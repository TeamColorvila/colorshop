...
class CS_Tax {
	...
	public function find_rates( $args = array(), $deprecated_state = null, $deprecated_postcode = null, $deprecated_class = null ) {
		...
		// Make sure the arguments match the CS 2.0 structure
		if ( is_string( $args ) ) {
			_deprecated_argument( __CLASS__ . '->' . __FUNCTION__, '2.0', 'Use $args["country"] instead. Deprecated argument will be removed in CS 2.1.' );
			$args = array(
				'country' => $args
			);
		}
		...
		if ( func_num_args() > 1 ) {
			if ( null !== $deprecated_state ) {
				_deprecated_argument( __CLASS__ . '->' . __FUNCTION__, '2.0', 'Use $args["state"] instead. Deprecated argument will be removed in CS 2.1.' );
				$args['state'] = $deprecated_state;
			}

			if ( null !== $deprecated_postcode ) {
				_deprecated_argument( __CLASS__ . '->' . __FUNCTION__, '2.0', 'Use $args["postcode"] instead. Deprecated argument will be removed in CS 2.1.' );
				$args['postcode'] = $deprecated_postcode;
			}

			if ( null !== $deprecated_class ) {
				_deprecated_argument( __CLASS__ . '->' . __FUNCTION__, '2.0', 'Use $args["tax_class"] instead. Deprecated argument will be removed in CS 2.1.' );
		...
		// Handle postcodes
		$valid_postcodes 	= array( '*', strtoupper( colorshop_clean( $postcode ) ) );

		// Work out possible valid wildcard postcodes
		$postcode_length	= strlen( $postcode );
		$wildcard_postcode	= strtoupper( colorshop_clean( $postcode ) );
		...
		$found_rates = $wpdb->get_results( $wpdb->prepare( "
			SELECT tax_rates.* FROM
				{$wpdb->prefix}colorshop_tax_rates as tax_rates
			LEFT OUTER JOIN
				{$wpdb->prefix}colorshop_tax_rate_locations as locations ON tax_rates.tax_rate_id = locations.tax_rate_id
			WHERE
		...
		/*
		echo '<pre>' . __( 'Matched tax rates:', 'colorshop' );
		var_dump( $matched_tax_rates );
		echo '</pre>';
		*/

		return apply_filters( 'colorshop_matched_tax_rates', $matched_tax_rates, $country, $state, $postcode, $city, $tax_class );
	}
	...
	public function get_rates( $tax_class = '' ) {
		...
		if ( ( defined('COLORSHOP_CHECKOUT') && COLORSHOP_CHECKOUT ) || $colorshop->customer->has_calculated_shipping() ) {

			list( $country, $state, $postcode, $city ) = $colorshop->customer->get_taxable_address();
			...
			// The colorshop_default_customer_address option (when set to base) is also used here.
			$matched_tax_rates = $colorshop->cart->prices_include_tax || get_option( 'colorshop_default_customer_address' ) == 'base'
				? $this->get_shop_base_rate( $tax_class )
				: array();

		}

		return apply_filters('colorshop_matched_rates', $matched_tax_rates, $tax_class);
	...
	public function get_shipping_tax_rates( $tax_class = null ) {
		...
		if ( $shipping_tax_class = get_option( 'colorshop_shipping_tax_class' ) ) {
		...
		if ( ( defined('COLORSHOP_CHECKOUT') && COLORSHOP_CHECKOUT ) || $colorshop->customer->has_calculated_shipping() ) {
			...
			// Prices which include tax should always use the base rate if we don't know where the user is located
			// Prices excluding tax however should just not add any taxes, as they will be added during checkout
			if ( $colorshop->cart->prices_include_tax || get_option( 'colorshop_default_customer_address' ) == 'base' ) {
				$country 	= $colorshop->countries->get_base_country();
				$state 		= $colorshop->countries->get_base_state();
				$postcode   = '';
				$city		= '';
		...
		if ( ! is_null( $tax_class ) ) {
			...
			if ( sizeof( $colorshop->cart->get_cart() ) > 0 )
				foreach ( $colorshop->cart->get_cart() as $item )
	...
	public function calc_tax( $price, $rates, $price_includes_tax = true, $suppress_rounding = false ) {
		...
		if ( $price_includes_tax ) {
			...
			foreach ( $rates as $key => $rate ) {
				...
				$tax_amount = apply_filters( 'colorshop_price_inc_tax_amount', $tax_amount, $key, $rate, $price, $non_compound_price );

				// Back to pounds
				$tax_amount = ( $tax_amount / 100 );

				// Rounding
				if ( get_option( 'colorshop_tax_round_at_subtotal' ) == 'no' && ! $suppress_rounding ) {
		...
		// Prices ex tax
			...
			foreach ( $rates as $key => $rate ) {
				...
				// ADVANCED: Allow third parties to modify this rate
				$tax_amount = apply_filters( 'colorshop_price_ex_tax_amount', $tax_amount, $key, $rate, $price );
			...
			if ( $rates ) {
				foreach ( $rates as $key => $rate ) {
					...
					$tax_amount = apply_filters( 'colorshop_price_ex_tax_amount', $tax_amount, $key, $rate, $price, $the_price_inc_tax, $pre_compound_total );
		...
		return apply_filters( 'colorshop_calc_tax', $taxes, $price, $rates, $price_includes_tax, $suppress_rounding );
	}
	...
	public function is_compound( $key ) {
		global $wpdb;
		return $wpdb->get_var( $wpdb->prepare( "SELECT tax_rate_compound FROM {$wpdb->prefix}colorshop_tax_rates WHERE tax_rate_id = %s", $key ) ) ? true : false;
	}
	...
	public function get_rate_label( $key ) {
		global $wpdb;
		return apply_filters( 'colorshop_rate_label', $wpdb->get_var( $wpdb->prepare( "SELECT tax_rate_name FROM {$wpdb->prefix}colorshop_tax_rates WHERE tax_rate_id = %s", $key ) ), $key, $this );
	}
	...
	public function get_rate_code( $key ) {
		...
		$rate = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}colorshop_tax_rates WHERE tax_rate_id = %s", $key ) );
		...
		return apply_filters( 'colorshop_rate_code', strtoupper( implode( '-', array_filter( $code ) ) ), $key, $this );
	}
	 ...
	 * }
	 * add_filter( 'colorshop_tax_round', 'euro_5cent_rounding' );
	 *
	 */
	public function round( $in ) {
		return apply_filters( 'colorshop_tax_round', round( $in, $this->dp ), $in );
	}
