		...
		if ( ! empty( $this->meta ) ) {
			...
			foreach ( $this->meta as $meta_key => $meta_values ) {
				...
				foreach( $meta_values as $meta_value ) {
					...
					if ( $flat )
						$meta_list[] = esc_attr( $colorshop->attribute_label( str_replace( 'attribute_', '', $meta_key ) ) . ': ' . $meta_value );
					else
						$meta_list[] = '<dt>' . wp_kses_post( $colorshop->attribute_label( str_replace( 'attribute_', '', $meta_key ) ) ) . ':</dt><dd>' . wp_kses_post( $meta_value ) . '</dd>';
