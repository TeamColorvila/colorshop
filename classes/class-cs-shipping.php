...
class CS_Shipping {
	...
	/** @var array Stores methods loaded into colorshop. */
     ...
     */
    function init() {
		do_action( 'colorshop_shipping_init' );

		$this->enabled = ( get_option('colorshop_calc_shipping') == 'no' ) ? false : true;
	}
	...
	function load_shipping_methods( $package = false ) {
		...
		do_action( 'colorshop_load_shipping_methods', $package );

		// Register methods through a filter
		$shipping_methods_to_load = apply_filters( 'colorshop_shipping_methods', array() );
	...
	function sort_shipping_methods() {
		...
		// Get order option
		$ordering 	= (array) get_option('colorshop_shipping_method_order');
		$order_end 	= 999;
	...
	function calculate_shipping( $packages = array() ) {
		...
		// Get chosen method
		if ( ! empty( $colorshop->session->chosen_shipping_method ) )
			$chosen_method = $colorshop->session->chosen_shipping_method;

		$colorshop->session->available_methods_count = sizeof( $_available_methods );
		...
		if ( sizeof( $_available_methods ) > 0 ) {
			...
			if ( empty( $chosen_method ) || ! isset( $_available_methods[ $chosen_method ] ) ) {
				...
				$chosen_method = apply_filters( 'colorshop_shipping_chosen_method', get_option('colorshop_default_shipping_method'), $_available_methods );
				...
				// Store chosen method
				$colorshop->session->chosen_shipping_method = $chosen_method;

				// Do action for this chosen method
				do_action( 'colorshop_shipping_method_chosen', $chosen_method );
			}
	...
	function calculate_shipping_for_package( $package = array() ) {
		...
		$package_hash = 'cs_ship_' . md5( json_encode( $package ) );
		...
		if ( false === ( $stored_rates = get_transient( $package_hash ) ) ) {
			...
			$package['rates'] = apply_filters( 'colorshop_package_rates', $package['rates'], $package );
	...
	function get_available_shipping_methods() {
		...
		return apply_filters( 'colorshop_available_shipping_methods', $available_methods );
	}
	...
	function reset_shipping() {
		global $colorshop;
		unset( $colorshop->session->chosen_shipping_method );
		$this->shipping_total = 0;
		$this->shipping_taxes = array();
		$this->shipping_label = null;
		$this->packages = array();
	...
	function process_admin_options() {
		...
		update_option( 'colorshop_default_shipping_method', $default_shipping_method );
		update_option( 'colorshop_shipping_method_order', $order );
	}
 ...
 * @since 1.5.7
 */
function colorshop_register_shipping_method( $shipping_method ) {
	$GLOBALS['colorshop']->shipping->register_shipping_method( $shipping_method );
}